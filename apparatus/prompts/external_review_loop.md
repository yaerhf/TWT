...
4. **ADJUDICATE** findings via §8a — an external review IS a checker verdict: engine
   ...
   **R11 — A DECLINE RECORDS ITS REVERSAL CONDITION (RUL-109, human coordinator,
   2026-08-27).** Every DECLINED entry written in an adjudication carries, in the same
   entry, the condition under which the decline would flip — dated where the condition is
   an act ("holds until the F3 body-level fetch of X, owed by <date>"), named where it is
   a computation or measurement. The symmetric ground: R7 gates adoptions and records the
   check WITH the adoption; before this rule nothing required a decline to record what
   would reverse it, and the first sweep ever pointed at declines found three resting on
   future acts no record dated (under-claim sweep V-B, 2026-08-27). A decline with no
   statable reversal condition says so explicitly — "unconditional on current record" is
   itself the recorded condition, and it is a strong claim the sweep may attack.
