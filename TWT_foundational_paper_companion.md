...
*Total primitives in the engine: 369 as of 2026-08-21 (two files since the engine split — `twt.py` MAIN: 292 module-level defs = 258 public + 24 helpers prefixed `_` + 10 classes; `twt_companion.py` COMPANION: 77 defs = 64 public + 13 helpers; re-counted by AST 2026-08-21, not incremented. Refresh this count+date at each banking pass, and count rather than increment). The table below lists the
246 load-bearing-primitive rows (re-counted 2026-08-21, not incremented; 246 distinct primitives —
the second, byte-identical `B_minus_L_anomaly` row was removed at the 2026-08-18 archivist
pass, so rows and primitives agree; `weak_su2_menu_exhaustion`, `charge_assignment_from_anchor` and `charge_sector_provenance` added 2026-08-21) that V3 R-NNNs cite
