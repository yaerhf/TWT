...
| R-005 | Defect's two faces (spatial winding + meta-time rotor) coupled by I_4 Hodge duality | DERIVED-STRUCTURAL | I4_maps_L_to_Q | A.3 | R-004, R-010 | R-007, R-053 | **REWORDED 2026-08-27 (RUL-110 / V3-1b, blast-radius BREAK-3):** the two faces are **algebraically independent and dynamically locked** — whether the lock is 1:1 is a separate, kernel-level question (§D.5). The former gloss *"the two faces are not independent observables"* is WITHDRAWN as a wording: under the adopted drive-referenced two-rate defect rotor the two plane angles are independently variable, and the row's algebraic content (`I4_maps_L_to_Q`, a Hodge interchange theorem) is untouched by the change — only the gloss moved. Sector-split (2026-07-02 sweep, R-127/R-128): the observer-visible winding↔mass-phase-axis relation is identity in L (lepton), I₄-dual in Q (quark); R-005's coupling is substrate-level. |
...
| R-037 | Sakharov induced EH: `G_N⁻¹ ~ N_eff Λ²/(12π)`; `Λ_S = √(2π) M_Pl` (scheme) · `Λ_L = 1/a ∈ [0.39, 0.73] M_Pl` (which-Λ ruled 2026-07-30) | DERIVED-generic-given-4D | sakharov_induced_gravity + induced_G_bracket_mode_count + induced_G_only_monad_scale_enters + induced_G_leading_coefficient_mass_independent + induced_G_quadratic_divergence_from_4D | B.6 | **R-102** (N_eff = 6 IS dim of the local state space), R-036, A-1a | R-038, R-041 | The Λ² scaling is generic-given-4D, not a dynamical derivation. Absolute magnitude #1-gap-gated. **RESOLVED (2026-07-29) + WHICH-Λ RULED (coordinator, 2026-07-30).** History: the 2026-07-28 pass widened the bracket to `[0.13, 2.5] M_Pl` over (i) a determinate unit-convention fix — the engine's `Lambda_over_MPl = 4π` is against the REDUCED `M_red = M_Pl/√(8π)`, i.e. `2.51 M_Pl` non-reduced — and (ii) an apparently UNRECONCILED three-way `c_reg` disagreement. The 2026-07-29 pass RESOLVED (ii): ONE coefficient, `c_reg = 1/12` exactly in the proper-time variable; `≈1.8 = c_lat/12` is the SAME computation written in `Λ := 1/a` (R-163), and `~1` was a never-computed placeholder. The which-Λ ruling then SPLIT the symbol — `Λ_S = √(2π) M_Pl` (scheme; Sakharov/`G` bookkeeping only, no substrate information) vs `Λ_L = 1/a ∈ [0.39, 0.73] M_Pl` (every lattice-dispersion consumer; band from OA-LF-ii `κ ∈ [½, 2]` on `c_lat = 21.83`) — and RETIRED the wide bracket. Naive `η⁽⁴⁾` re-cut to `[1.9, 6.7]`; exclusion E21-split 2026-08-26 (~1 order unconditional / 6–7 pure-proton-conditional; 3–9 retired — VG-6). |
...
| R-040 | Induced G sign positive: spin-2 spectral positivity (`C_T > 0` by unitarity) — **and the equivalence with substrate stability is WITHDRAWN**: the sign rides on `C_T > 0` alone, a unitarity statement about the matter sector, not on the ordered phase being a global minimum. Substrate stability is a separate and now BOUNDED question (§D.4.3 / N70: the reference state is a genuine local minimum only against modes below a long-wavelength threshold) | DERIVED-sign-only | induced_G_sign_cross_check | B.6 | R-037 | — | **The “two-pillar unification — same physics in different language” gloss is WITHDRAWN** (a biconditional cannot stand once one leg is bounded); the result is DERIVED-sign-only as before, no tier moves and no number moves, and it now rests on LESS. Still removes the RF-1 falsifier, by `C_T > 0` alone. |
...
| R-160 | Born exponent = 2 as a theorem: F2+F3 ⇒ additivity (coarse-graining) ⇒ Gleason ⇒ `Tr(ρP)`; F4 ⇒ `ρ = \|ψ⟩⟨ψ\|`. No power-law family assumed anywhere; dim-2 sectors inherit through the joint system–detector sector | DERIVED-conditional-on-(F1–F4) + import-exempt pure math (Gleason 1957) | born_exponent_gleason_closure | B.3.3 | R-021, R-023, R-029 | R-027; **R-184 — CROSS-WIRE, NOT A DEPENDENCY: independently corroborated by a non-Gleason route, which reaches this row's CONCLUSION and not its chain (F1–F4 absent there, Gleason unused; one-way — it fixes no exponent until the empirical third-order null is adjoined, a datum this chain does not need)** | **F2** (statistical noncontextuality of the Role-3 selection functional) is the single NEW substantive premise; it does NOT follow from §B.3.1 frame-equivalence — covariance ≠ noncontextuality (engine counterexample). F1 carries single-outcome definiteness; F3 = total function on the JOINT lattice incl. entangled contexts (not "mild"). The coarse-graining reduction is literature-standard, not TWT-novel. Supersedes R-023's "plausibility-modulo-degree". **CARRIER-BACKGROUND INHERIT-NOTE (RUL-035; K-O1 keeper L2):** F3's total-function premise fails on the comb `x₄ = (2n+1)π/(2k_c)` under a reverse-referenced functional (all channels identically zero — no normalized weight exists there); under the ruled adjoint `t` the reference is carrier-free and F3 is unobstructed. Fires only when a carrier background is admitted into Part B; trivial-background content unchanged. |
| R-184 | **THE GENERAL-BILINEAR THIRD-ORDER NULL — an independent, non-Gleason route to R-160's conclusion.** **(T1) MASTER STRUCTURAL THEOREM (general `n`, PROVED not sampled — UNDER THE `P(∅) = 0` HYPOTHESIS, i.e. no slit-independent monomial; a constant `k` has `I₃ = k ≠ 0` while its all-three-touching part is 0, so the SK-F1 vacuum-offset channel sits OUTSIDE this theorem — the closing review's computed catch, 2026-08-25, and the arc's own F1 finding stated as mathematics):** the three-slit inclusion–exclusion operator `I₃[P] = P(ABC) − P(AB) − P(AC) − P(BC) + P(A) + P(B) + P(C)` returns **exactly the all-three-slit-touching part of `P(ABC)`** — the superset sign-sums are `+1/0/0/+1` for `|T| = 3/2/1/0`, so only the all-three class and the hypothesis-excluded constant class survive; an all-three-touching monomial appears only in `P(ABC)`, coefficient +1. **(T2) THE NULL:** hence **any** detection functional bilinear in the slit amplitudes — on **any** finite-dimensional state space, with an **arbitrary** bilinear kernel (Hermiticity **not** assumed; **no locality** assumed, since a non-local `K(x,y)` only yields another arbitrary complex `M_ij`) — has every monomial of bidegree (1,1), touching at most two slits, and is **annihilated identically**. **(T3) THE HYPOTHESIS IS PART OF THE THEOREM, NOT A FOOTNOTE:** the kernel must be **slit-configuration-INDEPENDENT** (`M_ij`, not `M^S_ij`) — with a set-dependent kernel the null **fails**, computed as a live counter-case. **(T4) THE NULL IS SHARP:** `I₃[(|ψ|²)^m] ≠ 0` for every `m ≥ 2`, because the all-three-touching part of `|a+b+c|^{2m}` has strictly positive multinomial coefficients and cannot cancel — the planted nonlinear violation that makes (T2) a measurement rather than a tautology | **DERIVED-A GIVEN additive amplitudes + a slit-configuration-independent bilinear kernel — generic set-indexed inclusion–exclusion, *NOT* a Clifford/D4 identity.** The rider is mandatory (canon §5 derived-vs-generic; the reviewer's §1.4 requirement), in kind with `brannen_z3_harmonic_collapse_invariant`: every identity is exact and **none** is substrate-specific — it holds for any theory with additive amplitudes and bilinear detection, with **zero TWT content**. Exact ≠ substrate-specific | `bilinear_detection_third_order_null` — banked 2026-08-25 in `twt_core.py` (consumes no V3 pick; no `CORE_PROVENANCE` row — it rides no entered datum and no posited premise), 3 suite checks in `check_twt_observer_qm` (suite 494 → 497) | B.3.3 | — (**shares no premise with R-160**: F1–F4 absent, Gleason unused; the independence is the point) | **R-160 — CORROBORATION OF THE CONCLUSION ONLY, not of its chain**, and **one-way**: this row fixes no exponent by itself and reaches R-160's conclusion only with the **empirical** third-order null adjoined — a datum R-160 does not need | **PROVENANCE:** the Sorkin scaling arc, `knowledge/audit/sorkin_arc_2026-08-25/` — `PREREGISTRATION_FROZEN.md` (frozen and committed alone before any script), `SORKIN_SCALING_L1_2026-08-25.md` (history), `SORKIN_L2_2026-08-25.md` §§2.1–2.3, 2.7 (the operative claim register), and the three §8a verdicts in that directory, all cross-class to the arc's Fable authorship: `VERDICT_REVIEWER_SORKIN_L1.md` (Opus, contra-briefed — **proved** the general-`n` theorem the arc had only sampled at `n ≤ 3` (an UNDER-CLAIM repair, reproduced to `m = 7`/1107 terms), supplied the `d ∈ {3,6}` witnesses the L1 row mis-cited, and supplied the (T3) hypothesis together with the counter-case that shows it bites), `VERDICT_METAOBSERVER_SORKIN_L1.md`, `VERDICT_KEEPER_SORKIN_L1.md` (*"engine-bankable as stated. Bank it whatever happens to the rest."*). **WHAT IS NOT BANKED WITH IT** — and the arc's own L2 register is where those sit, at CANDIDATE: no `ρ_sat` scaling, no `κ_TWT`, no substrate intensity-exponent claim, no SK-P1 bounded-amplitude premise, no discharge of **F2** (the corpus's registered weakest premise), and **no coverage of the arc's live SK-F1 mode** — a slit-set-*dependent* vacuum/carrier offset, which is exactly what (T3) excludes by hypothesis and which remains UNCOMPUTED there. Prior-art note carried from the reviewer: the experimental Sorkin literature's 8-term/background-subtracted observable is flagged as probable prior art for the arc's SK-F1 finding (body-level fetch owed there) — it does not touch (T1)–(T4), which are combinatorial |
...
| R-165 | Lorentz-violation orders separated: D4's bond-set **fourth** moment is EXACTLY isotropic (`M_1111 = 12 = 3 M_1122`, residual 0), which `Z⁴` is not — so rotational anisotropy is pushed past dimension six to **dimension eight**, `(E/Λ_L)⁴ ≈ 2.0×10⁻³¹` at the loose corner of the ruled `Λ_L` band (`0.39 M_Pl`; which-Λ ruling 2026-07-30) and smaller still at its tight end; but the **rotationally invariant** dimension-six residual `η⁽⁴⁾ p⁴/M²_Pl` (equivalently `c·p⁴/Λ²`) is protected by neither R-016 nor D4 and is #1-gap GATED | DERIVED-A (the lattice moment identities AND the invariant-space dimensions, exact) + **DERIVED-conditional-on-(P-an ∧ P-pg ∧ P-gs ∧ P-op)** for the dimension-eight *inference* — **FULL NAMING, executed 2026-08-25 as the ONE coordinated engine+suite+companion edit the R-165 amendment (§4(i)) left owed. The string is held byte-identical to the engine's own `tier` literal (`d4_lattice_lorentz_violation_orders`, asserted in `twt_test.py`), and all four premises are now DEFINED in that primitive's returned `anisotropy_premises`, with the suite enforcing that the two sets agree exactly — so the tier names nothing the engine does not carry. The previous string named only the two headline premises and silently omitted (P-gs), added 2026-07-31; that omission is what the full naming repairs. NOT EXHAUSTIVE OF ALL CONDITIONS, and it says so in the literal: these four are the paper §E premise register's own list, and the §B.1.5 scalar-in-internal-index condition is a fifth named condition that register does not label. Nothing computed moves — no value, no count, no verdict: (P-op) can only make the POSITIVE half weaker, and R-165's negative half (the isotropic dim-6 exposure, E1, I-19, VG-6's isotropic row, N52's hole-location, the Λ_L band) is untouched a fortiori** + INPUT (published n = 4 bounds, import **I-19**) + GATED (the dim-6 isotropic coefficient itself) | d4_lattice_lorentz_violation_orders | B.1.5, B.6.3, B.6.4, E.3.3 VG-6, E.3.5(4) | R-004, R-016, R-112 | R-016, R-039, R-112 (`eom_constraint_class` H4 + E1) | **Two results in one, of opposite sign.** POSITIVE: the reason is REPRESENTATION-THEORETIC, not a kernel model (canon §3 — the nearest-neighbour-Laplacian argument was a model and is NOT what this rests on). `\|Aut(D4 root system)\| = 1152 = \|W(F4)\|`, whose invariant degrees `{2,6,8,12}` make the **degree-4 invariant space one-dimensional** (only `(k²)²`) — so symmetry forces the quartic isotropic for ANY point-group-symmetric analytic kernel, whatever the dynamics. The degree-6 space is 2-dimensional and D4's sixth bond moment is anisotropic (residual 12), so dim-8 is REACHED, not merely an upper bound — checked on both sides. Substrate-specific, not generic: `Z⁴`'s point group `B4` admits a 2-dimensional degree-4 space containing `Σk_i⁴` (and `N_1111 = 2`, `N_1122 = 0`, moment residual 2). **PREMISES, named not buried — (P-an)** analyticity in `k`, i.e. a derivative expansion exists (a non-analytic driven-dissipative memory kernel, the #1 gap itself, is NOT covered by a polynomial-invariant argument); **(P-pg)** the FULL point group INCLUDING TRIALITY — `W(D4)` (order 192) alone has a 3-dim degree-4 space, and the shell-2 sub-orbits `{±2e_i}` and `(±1,±1,±1,±1)` are each anisotropic (residual 32 apiece), cancelling ONLY at equal weight, so a substrate coupling weighting triality-related orbits unequally would RESTORE dim-6 anisotropy (N52 risk note); **(P-gs)** the ground state preserves the point group (added 2026-07-31, D-2): the §D.4.3 spiral vacuum BREAKS it — the species-universal `O(q²)` stiffness splitting is absorbed by the I-22 rescaling class, and the space-fixed (sidereal) residual is exactly SC-2's open question; **(P-op) THE OPERATIVE-SYMMETRY PREMISE** (added 2026-08-25, D4 standalone cold-review round — `knowledge/audit/standalone_reviews_2026-08-25/ADJUDICATION_D4_2026-08-25.md` §1.4; the amendment record is `R165_AMENDMENT_2026-08-25.md` in the same directory — internal record, not shipped): **the symmetry operative on the sector this row protects is the FULL point group `G` [1152], not the DRIVEN subgroup.** The theorem above is proved at the four-dimensional Euclidean point group, but the claim it is quoted for — leading *rotational* (spatial) anisotropy, read against SME anisotropic coefficients and sidereal cosmic-ray limits — is a **three**-dimensional statement, and the drive singles out `e₄`. COMPUTED (rebuilt independently of the engine's rig, this round): `Stab_G(e₄)` has order **48**, is block-diagonal, and restricts **faithfully and exactly onto `W(B₃)`** — all 48 signed 3-permutations — whose invariant degrees are `{2,4,6}`, so the **degree-4 spatial invariant space is TWO-dimensional**, spanned by `(k_sp²)²` **and `Σ_{i≤3} k_i⁴`. At the driven group the anisotropic spatial quartic is PERMITTED** (`Σ_{i≤3}k_i⁴` is non-invariant under `G` but invariant under `G₄₈` — 0/48 group elements move it (the standalone's independent rebuild counted the same fact as 0/1440 element×probe checks at 30 probes per element); 1056/1152 full-group elements move it, forced by the witness's stabilizer being exactly `G₉₆` — see R-185). What then carries the protection at the driven group is **a full-group property of the coupling, not the driven symmetry** (R-185 re-key, 2026-08-25): the 24 bonds are a single equal-weight `W(F4)` orbit, forcing `Σ_b(b·k)⁴ = 12(k²)²` identically in four variables, so the *spatial* fourth moment is **exactly isotropic on every 3-plane** (`M_1111 = 12 = 3M_1122`, residual 0, full tensor) — a permission left unpopulated rather than a forbidden term. The 12 bonds lying in the `e₄ = 0` hyperplane alone give spatial residual `−4` and the 12 `e₄`-bearing bonds `+4`: the **e₄-axis sensitivity decomposition** of that protection, not its mechanism (at a generic axis the split does not exist and isotropy persists). **WOULD-CHANGE-IF:** any driven-sector term populating the permitted invariant — a coupling that survives at `G₄₈` but not at `G` and carries a nonzero spatial fourth moment (equivalently, any reweighting that breaks the `±4` cancellation) — restores **dimension-six spatial anisotropy**, facing SME-type sidereal bounds rather than the isotropic ones. **This premise is the group-theoretic content of the arbitration the §D.5.7 Γ-survivor pointer (below) dockets:** the candidate answer to "which premise does a nonzero driven-group Γ survivor evade" is that it evades neither (P-an) nor (P-pg) but **(P-op)**. Nothing about the banked value, tier arithmetic or exposure count moves on this amendment; it is strictly a scope statement made explicit. NEGATIVE: the isotropic dim-6 term is unprotected, and at dimension six species-universality is **not** a symmetry (the induced velocity shift `≈ (3/2)η p²/Λ²` is momentum-dependent, so no rescaling removes it; Stecker's constrained combination is `η_π − 25η_p`, i.e. `−24η` for universal `η`, not zero). Setting the SUBSTRATE coefficient `c` to unity gives `η⁽⁴⁾ = c_lat/(2π) ∈ [1.9, 6.7]` (ruled `Λ_L = 1/a` band) — excluded UNCONDITIONALLY by ~1.1–1.7 orders (Auger 2022 model-independent hadronic, superluminal branch) and by 6.3–6.8 orders only under pure-proton composition, the old 3–9 figure RETIRED (E21 re-cut 2026-08-26, `BOUNDS_DOSSIER_V-C_2026-08-26.md`) — reported ONLY to size the exposure, never as a prediction. **(Λ NOTE, 2026-07-28 → 2026-07-30: the 2026-07-28 pass widened the bracket to `Λ ∈ [0.13, 2.5] M_Pl` over a reduced-vs-non-reduced Planck-mass unit fix PLUS what was then read as an unreconciled three-way `c_reg` disagreement. The 2026-07-29 pass CLOSED the `c_reg` half: `c_reg_from_substrate_mode_content` computes `a_1 = R` on TWT's own linear face, giving one value `c_reg = 1/12`, with `≈1.8` the same coefficient written in `Λ := 1/a` and `~1` a never-computed placeholder. The coordinator's 2026-07-30 which-`Λ` ruling then assigned the lattice-dispersion quantities that produce THIS exposure `Λ_L = 1/a ∈ [0.39, 0.73] M_Pl` and RETIRED the wide bracket — this row's numbers are re-cut accordingly. Two caveats stand: the `c_reg` settlement is ONE-PASS ONLY (its adversarial verifier died mid-response) and it is tier-CAPPED by R-041, whose shift symmetry — not the substrate dynamics — is what excludes the conformal corner. See R-037.)** The defect form factor supplies `(f_π/m_p)² ~ 10⁻²` — marginal against the ~1-order unconditional gap (clears 1.18× at ANW `f_π`, fails 1.77× at `F_π = 186`), far short of the 6–7-order conditional corners (E21) — and supplies NOTHING for the photon, which §B.5.4 makes a **bulk** strain mode with no internal structure — the most exposed sector, not the exempt one. NORMALIZATION (the two conventions must not collide): `η⁽⁴⁾` is the coefficient of `p⁴/M²_Pl` (Liberati); the substrate's own form is `c·p⁴/Λ²` with `c = O(1)`; hence `η⁽⁴⁾ = c·(M_Pl/Λ)²`, and "natural coefficient unity" means `c = 1`, NOT `η⁽⁴⁾ = 1`. FRAME: the **inertial**-frame question is a coherence WIN (the bounds are CMB-frame and the framework's τ₅-foliation IS the comoving frame, §B.4.5); the **jurisdiction** question is separate and hedged — these are INSIDE-frame inferences bounding an OUTSIDE-frame object through the un-built outside↔inside projection, so the exposure is named per canon §0a while its BINDINGNESS stays conditional (I-19 premise (e); same hedge as §E.3.1 rows 7–8). Supersedes the pre-2026-07-27 §B.1.5/§B.6.3 gloss and DELETED §E.3.1 rows 1–2 (documented V1→V2 drift, not fabrication: V1's "`10⁻¹⁵` to `10⁻²⁰`" dim-4 range had its **loose** end promoted to "the tightest matter-sector bound"). Negatives ledger **N52**; adjudication record `knowledge/audit/UHECR_VERDICT_2026-07-27.md`. **PRIOR ART, added 2026-07-29 — the physics is NOT original here.** The D4/F4 lattice's suppression of rotational cutoff effects relative to the hypercubic lattice is established lattice field theory: Neuberger, *Spinless fields on F(4) lattices*, Phys. Lett. B **199**, 536 (1987); used since by Celmaster, by Bhanot-Bitar-Heller-Neuberger, and by Klomfass; and live today in Katz & Nogradi, *QCD on the 16-cell honeycomb* (arXiv:2512.10604, Dec 2025), which states the dispersion form directly (first failure at `O(a⁴)`, not `O(a²)`). The 24-cell as a spherical 5-design is classical (Delsarte-Goethals-Seidel 1977). Both citations verified (INSPIRE / arXiv API). What R-165 claims as its own is NARROWER: the kernel-independent invariant-ring form (covering improved actions and radiative corrections at once), the two-sided sharpness (dim-8 REACHED), the explicit (P-pg) triality premise, and the transfer into LIV-EFT language. An external reviewer's "genuinely new and publishable on its own" verdict does NOT survive the lattice literature. Standalone treatment: `knowledge/corpus/D4_lattice_quartic_isotropy.md`. **POINTER (J,D/Γ rework, 2026-08-21): the trigger for re-examining this row's dimension-eight inference is the DRESSED entry coefficient of the driven-group Γ survivor — the named piece held at §D.5.7 (tree level gives `0/0` **under a simple or isoclinic defect twist — a conditioning that FIRED 2026-08-27, RUL-110 / V3-1b: the adopted two-rate twist is neither, so a bond-bilinear determination is open, owed and unbuilt, while the DRESSED value stays #1-gap-routed**; hence CANDIDATE by definition). No claim is adopted here about WHICH premise of the forced-isotropic-quartic theorem a nonzero survivor would evade: that question is under separate arbitration and this row records only the pointer.** **(UPDATE 2026-08-25: the arbitration's missing group-theoretic content is now supplied and named — see (P-op) above, `W(B₃)` at `e₄^⊥` with spatial degree-4 invariant dimension 2. The arbitration itself is NOT resolved here: (P-op) states the premise and its would-change-if, and observes that the premise bears on BOTH halves of this row at once rather than only the Γ-survivor half. No banked value moves.)** **(UPDATE 2026-08-25, same day, the banking pass: (P-op)'s group-theoretic content is now EXECUTABLE — R-185, `driven_group_spatial_invariants_wb3`. The premise text in this row's engine primitive points there.)** |
| R-185 | **THE (P-op) GROUP THEORY MADE EXECUTABLE — the driven group's spatial invariant theory.** Three facts, each exact: **(1)** `Stab_G(+e₄)` — the driven group, order 48, the same `G48` the Γ-referent rig builds — is block-diagonal and restricts **faithfully and exactly onto `W(B₃)`** (all 48 signed 3-permutations, set-equality of exact matrix keys both directions), with spatial invariant dimensions 1/2/3 at degrees 2/4/6 — the free polynomial algebra on `W(B₃)`'s **classical** invariant degrees `{2,4,6}` (Coxeter 1951; Chevalley 1955; Shephard–Todd 1954 — the executable check is what is banked, not the mathematics). **(2)** The **degree-4 SPATIAL invariant space at the driven group is 2-dimensional** — spanned by `(k_sp²)²` and `Σ_{i≤3}k_i⁴` — so an anisotropic spatial quartic is **PERMITTED** at the driven group though **FORBIDDEN** at the full group [1152] (4-variable degree-4 dims: 1 at `G`, 4 at `G₄₈`), computed by **two genuinely independent methods inside the primitive and asserted equal** — symmetric-power traces AND the Reynolds-operator projector rank (§8a repair: the originally-staged cross-tie to R-165's count was the SAME Molien formula re-run through an independently-cached group object, and is kept only as a labeled CONSISTENCY TIE). The witness `Σ_{i≤3}k_i⁴` is moved by **1056/1152** full-group elements, **0/48** driven-group and **0/96** axis-group ones — the 1056 is FORCED, `1056 = 1152 − 96`, since the witness's stabilizer in `G` is exactly `G₉₆` (exact coefficient transport, not sampling). **★ AXIS SUB-PREMISE, NAMED NOT RESOLVED (meta-observer F1):** all driven-group numbers sit at the maximally symmetric axis — `e₄` is a lattice symmetry direction, hence `\|Stab\| = 48`; a generic drive axis has trivial stabilizer and spatial degree-4 permission dimension **15**. The alignment *the advance axis is a lattice symmetry axis* is an implicit V3 commitment (V3-1-adjacent) this row now names; family-tree disposition is the coordinator's. **(3)** The 24-bond spatial fourth moment is **exactly isotropic at FULL TENSOR rank** (residual 0.0; the halves are residual-4 each, opposite in the anisotropy scalar: `+4` e₄-bearing / `−4` in-hyperplane). **MECHANISM, KEYED CORRECTLY (meta-observer F4, engine-arbitrated):** the isotropy is a **FULL-ORBIT fact** — one equal-weight `W(F4)` orbit forces `Σ_b(b·k)⁴ = 12(k²)²` identically in FOUR variables, and that descends to EVERY 3-plane (at a generic axis the 12+12 split does not exist and isotropy persists) — so the `+4/−4` split is the **e₄-axis SENSITIVITY DECOMPOSITION** of the protection, not its mechanism. What (P-op) adds: at the driven group nothing FORCES orbit-constancy — `G₄₈` symmetry permits the anisotropic quartic, and the protection is carried by the coupling remaining **constant on the full `W(F4)` orbit**, a full-group property the driven symmetry does not enforce. **WOULD-CHANGE-IF, keyed AXIS-INDEPENDENTLY:** any driven-sector coupling whose bond weighting is `G₄₈`-invariant but NOT constant on the `W(F4)` orbit restores dim-6 SPATIAL anisotropy (SME-type sidereal bounds); the executable exhibit runs in-process (`w = 2` hyperplane reweighting → `−4 ≠ 0` — an EXHIBIT entailed by the ±4 integers; the real negative control is the `G24` proper-subset check, seen to be a proper 24-element subset ≠ `W(B₃)`) | **DERIVED-A** (exact finite group theory: closure-built stabilizer, set-equality onto `W(B₃)`, invariant dimensions by two independent methods, integer bond moments). Consumes the D4 siting (**V3-1**) via `_gamma_bond_rig` — CANDIDATE half, correctly (canon §6 consumption rule). **RIDER (canon §5, R-184's exact-≠-substrate-specific class):** facts (1)–(2) are GENERIC reflection-group theory — the keeper computed that `Z⁴`'s point group `W(B₄)` has `\|Stab(e₄)\| = 48` restricting onto the SAME `W(B₃)`; the D4-specific content is the forbidden-at-`G`/permitted-at-`G₄₈` CONTRAST (`Z⁴` permits at both levels) and the 24-bond full-orbit isotropy with its ±4 sensitivity split | driven_group_spatial_invariants_wb3 — banked 2026-08-25 in `twt_candidate_v3.py`, 4 suite checks in `check_twt_cosmo` (suite 497 → 501), the `G24` negative control and the reweighting exhibit both run in-process | B.1.5 (via the §E premise register's (P-op) row) | R-165 (whose premise (P-op) this makes executable), V3-1 | R-165 (its (P-op) premise text now points here); RUL-105 (its revert list names this row — R-185 SURVIVES a (P-op) reversal as mathematics; what would be struck is its motivation and the R-165 pointer, not the primitive) | **SCOPE — NOTHING MOVES, and COMPUTED ≠ DISCHARGED**: no value, tier or count changes on this bank; (P-op) remains an OPEN assumption about the dressed driven dynamics; R-165's own asserts remain full-group statements and are correct as such; (P-op) can only make R-165's POSITIVE half weaker, its negative half untouched a fortiori. **The §D.5.7 Γ-survivor arbitration stays OPEN** — this row supplies its group-theoretic content as computation and does not resolve it. **PROVENANCE:** prose source `knowledge/audit/standalone_reviews_2026-08-25/ADJUDICATION_D4_2026-08-25.md` §1.4 (the D4 standalone cold-review round's independent computation); docket row "THE (P-op) GROUP THEORY — BANK IT" (2026-08-25 archivist pass, `TRIAGE_2026-08-25.md` §5.2). **What made it urgent:** the R-165 tier literal names (P-op) as a premise — a load-bearing premise asserted in a tier string deserves an executable form (*bank-before-you-cite one step removed*, canon §2), repaired by computing it where it is asserted. **§8a (2026-08-25, cross-class Opus vs Fable authorship, all three roles):** reviewer HOLDS-with-repairs (facts ≥0.999; found the false "two independent methods" cross-tie — repaired by adding the real Reynolds method; found the reweighting "plant" mislabeled — relabeled EXHIBIT; UNDER-CLAIM repaired: full-tensor isotropy banked, stronger than the staged scalar) · meta-observer UNDER-CLAIM/computed (F1 axis sub-premise — recorded; F4 mechanism re-key — adopted) · keeper COLLISION-repaired-same-pass (counts; RUL-105 revert list; the papers' four-vs-five premise lists) — verdicts + consensus at `knowledge/audit/pop_banking_2026-08-25/` |
| R-186 | **THE 24-BOND SCALAR DISPERSION QUARTIC — proposed by the round-5 EXTERNAL reviewer, three independent routes agree.** For the scalar nearest-neighbour symbol on the 24 D4 kissing bonds with `J_a` on the 12 `e₄`-bearing and `J_p` on the 12 in-hyperplane bonds (the two `G₄₈` sub-orbits of the one `W(F4)` orbit, R-185), the dispersion `F = 0` at `k₄ = iω` has, exactly (`r := J_p/J_a`): `c² = (1+2r)/3`; degree-4 isotropic `(1−r)(5+4r)/108`; degree-4 anisotropic `−(1−r)/36`. **At the orbit-constant point `r = 1`: `c² = 1` and BOTH degree-4 coefficients vanish identically** — `ω² = k_sp²` exactly through quartic order, leading survivor the degree-6 anisotropic `(1/90)[(k²)³ − Σk_i⁶]` (dimension EIGHT) — and `r = 1` is the UNIQUE root of either coefficient: one condition, both loads (R-185's orbit-constancy carrying the isotropic and anisotropic exposures at once). At the sub-orbit extreme `r = 0` (the V3-2a support shape): `5/108` and `−1/36`, both order `10⁻²` — lethal against the 3-to-9-order requirement if the dressed coupling sat there | **DERIVED-A OF THE STATED MODEL + MANDATORY MODEL RIDER** (the proposing reviewer's own fences, kept): scalar NN toy, NOT the banked {J, D} rotor structure; matrix-valued internal-index kernels outside the theorem (P-sc); non-analytic dissipative kernels outside entirely (P-an). PROOF OF CONCEPT, NOT DISCHARGE — VG-6/N52 unmoved, no banked exposure number changes. Consumes V3-1 and reads on V3-2/V3-2a — CANDIDATE half | d4_scalar_dispersion_quartic_coefficients — banked 2026-08-25 in `twt_candidate_v3.py` (exact Fraction series, no floats), 3 suite checks in `check_twt_cosmo` (suite 501 → 504) | B.1.5-adjacent (Core §5.3's sharpened natural-value statement) | R-185 (the sub-orbit split), V3-1, V3-2/V3-2a (what `r` reads on) | the Core §5.3 refinement; the docketed orbit-constancy computation | **PROVENANCE — the standing-external-recomputation datum:** proposed and first computed by round-5 **RETURN 02** (Opus class, 2026-08-25, `external_review_r5_2026-08-25/review_r5_02_opus_core_warm.md`); independently reproduced by the coordinator's sympy series the same hour; this primitive is the third route, in exact rational arithmetic. **PROVENANCE RE-KEYED 2026-08-26 (external follow-up, self-reported and adopted): the round-5 returns are NOT one persisting reviewer.** The instance that produced the full report is a FRESH context with no memory of the earlier returns and said so unprompted; identity across returns is NOT established and must not be asserted. Each return is credited to ITS OWN return, by class and date. What accumulates across the round is the RECORD, not the reviewer — and merging two instances under one identity is the referent error the checker roles exist to catch (the mathematics was unaffected; the attribution was wrong). **The named OPEN computation this row does NOT perform:** whether the dressed couplings sit at `r = 1` — the {J, D} matrix-valued version is the real object (worklist priority row; §D.5.7 adjacency) |
| R-187 | **THE K = 2/3 IDENTITY IS BRANCH-RESTRICTED — proposed by the round-5 external reviewer (warm return 05), the second externally-proposed banked result.** In the parametrization `√m_k = M(1 + √2·cos(θ + 2πk/3))`, K = 2/3 holds identically ONLY on the branch `min_k cos(θ + 2πk/3) > −1/√2` — exactly ONE QUARTER of the phase circle (three π/6 arcs, edges at the centres ± π/12, closed-form); off-branch the physical square root takes absolute values and K falls to K_min = 0.409365 (closed form at θ = π, matched by global scan). **A fact the verification added:** the physical phase θ = 2/9 sits only **0.0396 rad** inside the π/12 edge (minimum factor +0.040, ~15% of the arc half-width) — carried as a **noted non-coincidence** (canon move 5), no meaning assigned, numerology guard at full strength. The reviewer's framing — the arc-ratio reading should NAME the −1/√2 boundary; the branch condition as candidate content for the Koide geometric characterization — is CANDIDATE, not adopted | **DERIVED-A of the parametrization** (exact identity domain, closed-form measure/edges/K_min); c = √2 stays the endorsed INPUT and nothing about K's tier or the fit's status moves | koide_branch_restriction — banked 2026-08-26 in `twt_core.py` beside `dft_K_from_r`, 2 suite checks (suite 505 → 507) | C.3.2–C.3.4 (the Koide sector; dossier §19.4-class) | the Koide parametrization (c = √2 ENDORSED), R-070-class DFT form | the §5.2 Koide row's geometric-characterization clause (candidate content, unadopted) | **PROVENANCE:** round-5 **RETURN 05** (Opus class), `external_review_r5_2026-08-25/review_r5_05_opus_core_warm.md`; verified same-hour; adjudicated at E23. **PROVENANCE RE-KEYED 2026-08-26 (external follow-up, self-reported and adopted): the round-5 returns are NOT one persisting reviewer.** The instance that produced the full report is a FRESH context with no memory of the earlier returns and said so unprompted; identity across returns is NOT established and must not be asserted. Each return is credited to ITS OWN return, by class and date. What accumulates across the round is the RECORD, not the reviewer — and merging two instances under one identity is the referent error the checker roles exist to catch (the mathematics was unaffected; the attribution was wrong). **AND THE SUGGESTIVE READING IS WITHDRAWN ON ITS OWN STATISTIC** (external follow-up, adopted 2026-08-26): the normalized distance-to-nearest-edge is `0.151`, uniform on `[0,1]` under a uniform prior — unremarkable, and nearer the edge than the middle, so it argues mildly *against* specialness; landing in the arcs at all is 1-in-4 and was found post hoc. Carried as a plain positional fact, not as a noted non-coincidence |
| R-188 | **THE CHARGE ORBIT IS EXACTLY BOOST-INVARIANT — commissioned by an external objection whose PREMISE was right and whose CONCLUSION is refuted.** An external review held that the trivector charge functional `Y[T] = ⟨T̃ e₄ T⟩` had never been tested under boosts (**true — no primitive existed**) and predicted it would fail, since boosts do not preserve grade. It is **exactly invariant**: for the banked grade-one hyperbolic `B = exp(ζe_a/2)`, `e₄` anticommutes with every spatial `e_a` and `B̃ = B`, so `B e₄ B = e₄` and `⟨(BTB)̃ e₄ (BTB)⟩ = ⟨T̃ e₄ T⟩`. Measured over 80 randomized (axis, rapidity) pairs on all four trivectors: worst deviation `3.6×10⁻¹⁵`, **off-`e₄` leakage exactly `0.0`** — the value domain is boost-CLOSED, not merely covariant | **DERIVED-A** (exact identity, randomized, planted violation seen to fire at `8.2`, orthogonal-axis control silent) **with a MANDATORY COST CLAUSE**: the invariance is bought by holding `e₄` FIXED — the preferred foliation — and is NOT generic, so it discharges the charge-orbit objection COMPLETELY and the frame-equivalence question NOT AT ALL | charge_orbit_boost_invariance — banked 2026-08-26 in `twt_core.py`, 3 suite checks in `check_twt_spectra` (suite 507 → 510) | C.2.7-adjacent | the trivector charge orbit (R-056/R-057), the banked grade-one boost realization | R-159's premise set (unchanged — no tier moves) | **The objection's supporting analogy is REFUSED on a category ground the corpus already holds:** family-tree exposure **E-1** concerns LEFT MULTIPLICATION on the spinor module, this is CONJUGATION on the algebra — different action, different object, so *same disease, adjacent organ* does not carry. **PROVENANCE:** round-5 consolidated external review §3.2; independently re-verified by the coordinator before banking, including the discovery that a first-draft planted violation was VACUOUS (reverse of a scalar-plus-vector is itself, so rotor-style conjugation is identical) — replaced with the spatial-reference plant that fires |
| R-189 | **THE TREE-LEVEL LINEARISED HEALING LENGTH OF THE CANTED REFERENCE STATE — and, found by its own contra-briefed review, THE REFERENCE STATE IS NOT A LOCAL MINIMUM (N70).** Continuing R-174's Bloch stiffness to imaginary wavevector gives the static-propagator poles `det H(iκn̂) = 0`, `ξ = 1/Re κ` — the Ginzburg–Landau / chiral-magnet correlation length. Per gapped direction, per branch, in `ξ/a` (`a` = the D4 coordinate unit; §B.6.2 fixes the nearest-neighbour distance at `√2·a`): measured minimum **5.38834** [BRANCH: body-diagonal] / **5.42909** [BRANCH: axis], median **9.03** / **9.24**, p95 **27.9** / **25.6**, and **∞ on each branch's own helix axis** (the phason direction). ★ **The naive `√(12/g) ≈ 5.4` is the FLOOR, not the value** — it drops the odd (DM/Lifshitz) term `L`, which is comparable to `√(ρg)` on every spatial direction and vanishes identically only on `e4`; dropping it UNDER-estimates `ξ` (the Schur downfold is negative semi-definite), so the correction runs AGAINST the tension and the tension survives. ★ **N70/F1, the larger finding:** the lowest band has NEGATIVE curvature perpendicular to `k₀` on BOTH branches — `−0.02305291` [body-diagonal, NEW, non-single-`q`, fires N62's would-change-if (1)] and `−0.27180254` [axis, reproducing N62's own closed form to 7 significant figures, a re-tread] — so **no computed branch is a tree-level local minimum** and branch selection is fully kernel-routed. ★ The soft channel DOES supply a length (`2π/|k*|` = **1093.96** / **321.89 `a`**), replacing the withdrawn claim that a gapless channel could supply none | **DERIVED-numeric** (the lengths, the `±k₀` zeros, the F1 curvature, the F2 turnover — ALL branch-labelled) + **DERIVED-A** (`L(e4) = 0` identically; the `L = 0` reduction to `√(ρ/g)`) + **DERIVED-structural** (floor-not-value) + **FRAMING** (the cell-scale comparison). **MANDATORY CONDITIONING, and it travels with every quotation:** these are relaxation lengths about a state with a named tree-level instability (N70), valid as SHORT-DISTANCE readings inside a measured window — the state is a genuine local minimum against every mode of wavelength below **773.2 `a`** / **227.0 `a`**, one to two orders above `ξ`. They are NOT healing lengths back to a vacuum. **The measured minimum is a SAMPLED minimum, not a derived bound.** This is the STATIC Hessian, transverse-`k`=0 (planar) reading: NOT Bogoliubov (paraunitary — R-174's F2 caution stands, neither Bogoliubov IOU is discharged) and NOT a nonlinear soliton profile. Consumes the V3 `D/J = 0.787` calibration — CANDIDATE half | core_healing_length_canted_vacuum — banked 2026-08-27 in `twt_candidate_v3.py`, 10 suite checks in `check_twt_spectra` (suite 510 → 520) | D.4.3 / D.5-adjacent | R-174 (the Bloch stiffness and its rig), `canting_vacuum_branch_structure` (the two branches), KC-1 `spectral_branch_symmetry_class_filter` (the `{Γ, ±k₀}` gapless set — see provenance), N62 | N70 (new); N62 (its would-change-if (1) fired); PROBE C, whose named input this is | **PROVENANCE, and it splits three ways.** The probe is ours (frozen pre-registration committed ALONE before any script, `PREREGISTRATION_FROZEN.md` at `dcc6b12`). **N70/F1 and the F2 refutation were found by the CONTRA-BRIEFED §8a REVIEW of this probe, not by the probe** — the review reproduced every number, corroborated the method with a fourth independent route, then attacked the vacuum's own stability and found it. Both were independently re-verified here by routes the review did not use (an analytic degenerate-perturbation curvature; a 300-start band minimisation) before adoption. **One sub-claim was PUSHED BACK and the review WITHDREW it** on reproducing the developer's computation (the C7 re-attribution: the chain form is positive definite, 0 negative eigenvalues of 840, so the non-decay is gaplessness not negative stiffness) — recorded because a withdrawn reviewer finding is calibration data. **★ AND A NOVELTY OVER-CLAIM OF OUR OWN, SELF-CAUGHT AT BANK TIME AND WITHDRAWN:** the probe report called the `±k₀` gapless points "new to the record"; they are **not** — KC-1 already banks "the gapless set is exactly `{Γ, ±k₀}`, the helimagnet Goldstone triplet (**the EXISTENCE is this row's and has priority; the ATTRIBUTION is computed at R-189 check [9] as of 2026-08-27** — the twisted-generator symmetry map, two-way projections `1.0`, `H(2k₀)` gapped. "Triplet" counts gapless POINTS; R-189 counts MODES, 2+2+2 = the six broken global generators; `N_G = 2` is a Γ count and is untouched)" and **has priority**. What R-189 actually adds there is the ATTRIBUTION RAISED FROM ASSERTED TO COMPUTED (the explicit twisted-generator symmetry map, two-way projections `1.0`, `H(2k₀)` gapped) — R-174's own licence-to-identity pattern. `n_goldstone_canted_FM`'s `N_G = 2` (a Γ count) is UNTOUCHED. **The named OPEN condition:** every route runs on R-174's shared rig; a from-scratch commensurate-supercell energy check (side `~59`) is the condition of banking N70's body-diagonal leg, and is docketed, not claimed. Records: `knowledge/audit/probe_b_core_size_2026-08-27/` |
| R-190 | **THE MARGINAL QUARTIC'S RUNNING DOES NOT CARRY THE CORE SCALE TO THE CELL SCALE — and the honest form is "THE MECHANISM APPLIES AND DOES NOT REACH", never "the mechanism is absent".** PROBE C, the ★★★ docket item, frozen pre-registration, full ceremony. The sector's only log-runner is identified with the chiral `O(p⁴)` constant `l₂` — `λ_S ≡ 1/e² = 4l₂`, `l₁ = −l₂`, **derived in-suite from the `su(2)` trace identity and verified on held-out configurations at index ranges 2, 3 and 4**, not matched (the N42 guard) — giving `β_λS = μ d(1/e²)/dμ = −4γ₂/(16π²) = **−1/(6π²)**` from Gasser–Leutwyler `γ₂ = 2/3` read at primary with its hypotheses checked one by one (five match; the sixth, the domain of validity, **does not** — leg L-D). ★ **THE FLOW IS EXACT:** by Weinberg chiral counting `L ≥ 2 ⇒ D ≥ 6` and one `O(p⁴)` insertion `⇒ D ≥ 6`, so `β_{l₂}` carries **no power of `l₁` or `l₂` at any order in the chiral expansion** — exactly additive, exactly one-loop, **stronger than QCD's own β**, which takes two-loop corrections while this one does not. ★ **THE FLOW IS IR-FREE, HENCE IR-ANCHORED**, and that is the block: the free parameter sits at the **measured** hadronic end, so a hierarchy obtained this way is **read off**, not generated. Reach `= λ_S(cell)/c = 4π²/(e²γ₂)` = **1.9937 e-folds** at the banked `e = 5.45` (0.4422 at the measured ChPT value) against **36.68–42.00** required across R-189's eight branch- and direction-labelled anchorings (target `ℓ_S/a = 9.34e18`). A cell-end coupling that **would** work exists and is `O(1)` (`λ_S = 0.619–0.709`, `g² = e² = 1.41–1.61`) and is excluded by a **21×** conflict with the banked `e`, a **95×** conflict with the measured ChPT value (which runs the **wrong way**), and by lying **~25×** outside the loop-counting size `1/(16π²)` — outside the domain of the very β that would carry it | **DERIVED-A** (the Skyrme→`(l₁,l₂)` identification; the no-quadratic-part series result; the chiral-counting exactness theorem; the `γ₂`-cancellation vacuity demonstration) + **DERIVED-numeric** (`c = 1/(6π²)` given the READ `γ₂`; the e-fold reach — **normalization-invariant**, so the headline does not use the `λ_S = 4l₂` identification at all) + **FRAMING** (the cell-scale comparison, inheriting R-189's conditioning verbatim). **THE MAGNITUDE IS I-13-FREE** (an explicit one-loop computation, no analyticity/crossing/subtraction-count/optical theorem), and its **sign independently corroborates R-148** — a genuine two-route agreement, not a restatement; **R-148 itself stays CONDITIONAL on I-13 with its revert clause intact**. Consumes `E_PHYS`, `F_PI` and R-189 — CANDIDATE half | marginal_quartic_running_reach_to_cell_scale — banked 2026-08-27 in `twt_candidate_v3.py`, 16 suite checks in `check_twt_matter` (suite 520 → 536) | C.5.2 / D.5 | R-148 (the sign, and clause (4)'s ADDITIVE-not-antiscreening observation, which **has priority**), R-085 (the bare-coefficient positivity), R-189 (the anchoring lengths), `qcd_uv_conformal_phaseCD`'s `D2_AF_log_violations` (the only-log-runner statement), `induced_G_only_monad_scale_enters` (the `ℓ_S` referent) | N71 (new); the worklist's ★★★ PROBE C row (closed); §3.2's scale-class row (**explicitly does NOT move — "earned dimensionful scales" stays at 0**) | **PROVENANCE, AND IT CARRIES TWO CORRECTIONS AGAINST OURSELVES.** ★ **(1) THE FIRST BUILD'S STATED REASON WAS REFUTED and the block survives without it.** The L-1 first asserted *"an additive β has no pole and cannot be dialled, so the transmutation formula does not apply"* — **REFUTED by computation** in the contra-briefed §8a review and independently re-verified before acceptance: **every one-loop β is additive in the INVERSE coupling, QCD's included** (`d(1/α_s)/dlnμ = 2b₀`), and the probe's own positive control recomputed as an additive e-fold count returns `6.94530431891627`, **identical** to what it computed — the probe had validated its pipeline with the very formula it declared inapplicable. In the variable `e²` the flow reads `β(e²) = +c·e⁴` with a genuine pole. A second sentence fell with it: *"no coupling value works"* — one does, and lands **inside the pre-registration's own `[0.3, 3]` window**. Both are struck; what survives is the exactness theorem and the IR-anchoring, which is L-A's first clause. ★ **(2) A VACUOUS CHECK, CAUGHT AND SHIPPED AS A DEMONSTRATION** (canon §8a's memorised signature): the probe filed a third "control" claiming that `Λ_inv = μ·exp(λ_S/c)` landing on `Λ_χ` to 3 % corroborated **the magnitude of `c`**. **`γ₂` cancels identically**, as do the factor `4` and the scale `μ`, leaving `Λ_inv = M_π e^{l̄₂/2}` — verified at `γ₂ = 2/3, 1/3, 7.0, 0.01`, identical to 16 digits. *A tight tolerance on a quantity that cancels is a TELL, not rigour.* Withdrawn at every site, and **the cancellation is now asserted in the suite (check [10″]) so it cannot return**. The negative control was separately **tautological** and is replaced by a directional one; **run-validity rests on the positive control alone**, stated rather than absorbed into a three-way PASS. **REVIEW OUTCOME:** 13 accepts, **2 partial pushbacks, both ADJUDICATED ACCEPTED by the lead and both conservative** — P-A adds a computed clause (the UV-dial-vs-IR-dial asymmetry) converging with the review's own outcome-class finding; P-B adopts the review's under-claim in a **weaker** form than offered, because the review's own F5 (`l₂ʳ(2 GeV) < 0` with no pathology) makes `Λ_inv` a scheme-dependent LEC zero-crossing rather than a physical breakdown scale. Two under-claim repairs from the review made the block **stronger** (the domain-violation leg; the anti-transmuting form of the referent map). ★ **A MISFIT RECORDED AGAINST THE PRE-REGISTRATION ITSELF:** its outcome classes assumed a UV-anchored AF sector with a **microscopic dial**; this sector is IR-anchored, so *"the required microscopic `g²`"* is not a well-formed question here and **by the letter of the frozen classes the required `g²` lands in outcome (i)'s window** — what excludes (i) is L-A/L-B/L-C, not the value. (A second slip in the frozen document — its positive control naming `Λ_QCD/M_Z` as "~17 decades" when that ratio is ~2.6 — was **declared and corrected, both forms run**, never silently taken.) **PRIOR ART inside the corpus, credited:** R-148 clause (4) and anchor (0′)(a) already carry the additive-drift observation and the `ε⁴` series result; R-190's delta is the exactness **proof**, the **rate**, and the **e-fold count**. ★ **AND A NOVELTY OVER-CLAIM OF OUR OWN, SELF-CAUGHT AND WITHDRAWN (keeper O-3):** R-148's **step (3) CORROBORATION clause** already recorded **this very route** — verbatim, *“in one-loop ChPT the forward coefficient of this channel is pure `l2` … and `Γ₂ > 0` makes `l₂ʳ` decrease toward the UV — the same direction”* (witness, not load-bearing). **So the CONCORDANCE IS NOT NEW and R-148 has priority on it**; presenting *“a genuine two-route agreement”* as this row's finding **over-stated the delta**. What stands: the agreement is real, and R-190 **promotes R-148's witness from a named DIRECTION to a computed RATE**. The actual delta is untouched — the exactness proof, the rate, the e-fold count and its normalization-invariance, and the hypothesis check that found the domain failure. *(Precedent: R-189's own KC-1 novelty withdrawal, one commit earlier.)* ★ **AND `Λ_inv` IS NOT AN EARNED SCALE (keeper L-2):** the arc produces a dimensionful number in passing — `Λ_inv = M_π·e^{l̄₂/2} = **1.198 GeV**`, ~1.7× above `Λ_cell` — and it **earns nothing**: it is built from **two MEASURED inputs** (`M_π`, and an `l̄₂` not read at primary), and it is a **scheme-dependent LEC zero-crossing**, not a physical breakdown scale. **“Earned dimensionful scales” stays at 0**; what this row earns is a dimensionless **rate** and a dimensionless **e-fold count**. *If a future arc fixes `l̄₂` from structure — which is exactly what N71 handle (5) (HLS/VMD) reaches for — the comparative ledger's finding 20 is the first place to re-check the scale-class count.* **THE ONE LIVE ESCAPE, ARGUED AND UNVERIFIED — A POINTER, NOT A RESULT:** hidden local symmetry / vector-meson dominance (the `ρ` saturates `l̄₂`; the HLS gauge coupling **is** a marginal coupling with a propagator and a quadratic β). Records: `knowledge/audit/probe_c_transmutation_2026-08-27/` |
| R-191 | **KERNEL PROGRAMME GATE 1 — KC-1 RUN ON G-8, AND THE VERDICT IS NOT DECISIVE: G-8 IS THE FIRST KERNEL MEASURED TO PASS KC-1's NECESSARY CONDITION.** The G-8 CGL-class candidate (the first concrete kernel-class proposal; CANDIDATE), linearized about the drive-locked canted reference state as `L(k) = −(G + c·ad_B₀)H(k) + Ω₀·ad_B₀` (the probe's OWN operationalization, labeled), LEAVES the real class at fixed `k` in the strict `Ω⁻¹H` form on BOTH branches and BOTH dials (Im charpoly ~5e-3 vs controls ~5e-16; 50-digit `Im tr L² ≠ 0`; analytic reason: `tr(Hermitian × anti-Hermitian)` is purely imaginary) — **escape (a)'s first-order/precessional disjunct MEASURED-AND-FIRES**, so KC-1 no longer closes the spectral branch for G-8. The branch stays closed on: (i) the **Ω₀-free zero-set transfer** — `det L = det(−A)·det H` by inspection, zeros exactly `{Γ, ±k₀}`, with the c-DEPENDENT invertibility boundary (c ≠ 0: `ker G ∩ ker ad_B₀ = {0}`; c = 0: G PD outright); (ii) the fermionic and R-016 doors; with sufficiency UNMEASURED = **KG-2** (with drive the gapless set CONTRACTS `{Γ, ±k₀} → {Γ}` — ±k₀ lifted ~28 orders, Γ exactly preserved). ★ **ONE CONDITION, TWO CONSEQUENCES (the bank's parsimony):** G positive-definite on `ker(ad_B₀)` grants BOTH the zero-set transfer AND the drive-free N70 inheritance (Chetaev strictness on the exact energy identity `dE/dt = −(Hφ)†G(Hφ)`); its violation gives both failures at once, exactly marginal at every `k` (N72). ★ **THE DRIVEN SECTOR RE-SEATS THE REFERENCE STATE AT LINEAR ORDER AND IT SURVIVES `k → 0` — RE-CUT AT THE R-192 BANK (gate-2 keeper C-1/C-2):** the saturated law is the CLOSED FORM `max Re λ_slow = −γ·12J·k̃²(k)` (R-192's proven ⋆B₀ identity; the −0.5995 first banked here was the x = 14 reading, 0.087% below saturation; the ratio's closed form is `12/|curv|` = 520.5 body / 44.15 axis — twelve over N70's own banked curvature; the earlier "Ω₀-saturated, constant ratio ~516" was false in its four-digit clause and quoted the noisiest point as the limit — struck); the drive REMOVES the unstable direction; whole-BZ stable at (γ=0.05, c=0, Ω₀=0.7); **the destabilizing dial is the ALIGNED reactive channel c·Ω₀ > 0** (an unqualified "c" was forbidden by this row's own pairing — the anti-aligned half has NO reactive boundary, R-192); N70's would-change-if (3) has FIRED (first instance; LOCALIZED to a dial window by R-192) | **DERIVED-numeric** (branch- and D/J-labelled) + **DERIVED-A-by-inspection** (the factorization) + exact identities with conditioning classes (the drive-reversal pairing needs `M G Mᵀ = G`; the exhibited involution) + **CANDIDATE** (the kernel; the linear form is the probe's own). Rides N70's conditional tier (would-change-if (1) UNMET — this run is a SECOND READING ON THE SAME RIG and does not discharge it). CANDIDATE half | `g8_kernel_kc1_gate` — banked 2026-08-28 in `twt_candidate_v3.py`, 3 suite checks in `check_twt_matter` + the KC-1 fence check amended (suite 536 → 539); the same pass CLOSED KC-1's soft spot (M exhibited = the banked L/Q grading involution, exact 0.0 both branches, mutation-tested inline) | D.5 | R-175 (KC-1, the filter), R-174/`_estate_d4_magnon_rig` (the rig), R-189/N70 (the reference-state conditioning), I-29 (NOT discharged — G-8's Ω is dissipative, not paraunitary) | N72 (new); KG-2 (docketed); the S4-REAL node's door-1 line; gate 2 of the G-8 order | **FULL CEREMONY, and the first build's headline was REFUTED IN REVIEW AND ACCEPTED:** frozen prereg (1f27ea2, solo) → run → contra-briefed review (Opus) which refuted PASSED-CONSISTENT as a real-generator/real-class REFERENT SLIDE (the distinction TONGUES_L2 §4.5 had flagged, uncited) → developer reproduction of every counter-number → REV 2 re-hung on the honest legs → round-2 consensus (both directions: the reviewer conceded the zero-half transfer on the developer's computation; the developer conceded the c = 0 invertibility counterexample and the Γ exception) → meta-observer (prior art: von Neumann–Wigner/Herring; KTC/Krechetnikov–Marsden; the dial attribution) → keeper (6 collisions incl. ONE BANKING-STOPPER: the proposed R-189 "100%" requote was itself a referent error — the OLD result is EXACT in the k→0 limit and WINS; both referents now pinned in the primitive). **THREE VACUITY-TELL INSTANCES in one arc, all caught and struck** (the O(k) rank histogram — three exact doublets by construction; the projector tolerance; the det-multiplicativity tolerance). **The §4.5 "real generator" rewording is DECLINED with its RUL-109 reversal condition recorded** (flips if a genuinely symplectic/paraunitary #1-gap kernel is banked); both readings named in the primitive. **CLASS NOTE:** run authored Fable-class, all three checker passes Opus-class — cross-class satisfied keyed on authorship (RUL-065); the keeper's all-Opus assumption corrected on the dispatch record. Records: `knowledge/audit/g8_kc1_gate_2026-08-28/` |
| R-192 | **KERNEL PROGRAMME GATE 2 (LINEAR) — THE STABILITY PARTITION OF THE G-8 DIALS, EXACTLY TWO-DIMENSIONAL, WITH EVERY BOUNDARY PINNED AND LABELED.** The clock-scaling matrix identity `L(γ,c,Ω₀) = γ·L(1, c/γ, Ω₀/γ)` (DERIVED-GENERIC — degree-1 homogeneity, zero substrate content) makes every k-space object a function of `(c/γ, Ω₀/γ)`. **Boundaries:** drive threshold `x* = 0.01806 (body) / 0.06127 (axis)` (bisected roots, amp 1e-4; the `√(|curv|/a₂)` parabola is a LABELED TRUNCATION, 0.06%/0.7%); **chirality CONDITIONAL ON M-INVARIANT G** (R-191's banked conditioning): only the ALIGNED channel `c·Ω₀ > 0` destabilizes — `c* = γ·h(x)` by mode continuation (confirmed FROM BELOW by the reviewer; FIT ≈ 2.2√(γΩ₀), worst residual +19% quoted first), onset exactly the ⟨1,−1,0,0⟩ face-diagonal ray, type-I_s STATIONARY (reality FORCED on the ray at every dial) — while the anti-aligned half has **NO reactive boundary at all** (stable to |c| = 100). ★ **THE PROVEN SLOW BLOCK (the round's largest yield):** `P₀·ad_{B₀} = 0` exactly ⇒ `P₀LP₀ = −γP₀H(k)P₀` — an IDENTITY at every dial (the drive buys the DECOUPLING, Schur O(γ²/Ω₀)) — and the slow block is DIAGONAL in the fixed `{B₀, ⋆B₀}` basis with `⟨⋆B₀\|H(k)\|⋆B₀⟩ = 12J·k̃²(k)` EXACTLY at every k, both branches — **an extension of the banked DERIVED-A D = 0 identity** (pointer installed there), **supplying R-191's two unexplained constants in closed form** (A/γ = 12; ratio 12/\|curv\| = 520.5/44.15 — R-191's law row superseded in the same pass). The "drive erases DM" reading is scoped: ~99%/~86% of a uniform BZ-box sample, FAILING along k ∥ k₀ (body), where the B₀ diagonal entry governs. **Marginal-set identities:** Γ annihilated exactly; `L\|ker H(±k₀) = ∓iΩ₀·𝟙` for every (γ, c). ★ **THE SELECTION STRIP:** `x ∈ (0.01806, 0.06127)`, with its c-window, is where the driven kernel linearly selects **the very branch static energetics rejects** (both computed branches only; multi-q unscanned) — N70's wcif (3) LOCALIZED, riding its UNMET wcif (1) | **DERIVED-GENERIC** (the scaling) + **DERIVED-A-adjacent** (the proven slow block; the marginal-set identities; the rank statement with its C-4 three-fact bridge) + **DERIVED-numeric** (boundaries, branch- and D/J-labelled) + **FIT** (the 2.2√(γΩ₀) trend) + **CANDIDATE** (the kernel; the linear form is the probe's own operationalization). Rides N70's conditional tier. CANDIDATE half | `g8_gate2_linear_partition` — banked 2026-08-28 in `twt_candidate_v3.py`, 4 suite checks in `check_twt_matter` + the G1-3 fence re-cut + R-191's check [6] repaired (suite 539 → 543) | D.5 | R-191 (gate 1; its constants superseded here), R-174/`_estate_d4_magnon_rig`, R-189/N70/N72, the D = 0 identity (extended) | the nonlinear phase (staged, four-fence prescription binding); gate 3; KG-2; G-9 | **FULL CEREMONY, with THREE detector generations owned in-house** (the C-k control caught a mis-sampled soft direction and two under-sensitive c* detectors before review; the continuation instrument is authoritative, its collapse test its calibration). **REVIEW:** contra-briefed reviewer HOLDS-WITH-CORRECTIONS (three banking-stoppers: a vacuous N72-marginality check — the SIXTH vacuity instance in TWO days, with a SEVENTH found live in R-191's suite by the keeper and struck in this pass; artifact quadrant magnitudes; three inconsistent threshold pairs) + TWO free upgrades (the proven slow block; the marginal-set identity) with round-3 consensus on the diagonal-basis form; meta-observer: the strip's c-window scope-inflation (computed), the DERIVED-generic re-tier, KTC/Kapitza prior art carried. **KEEPER: 6 collisions, 4 resolving AGAINST YESTERDAY'S R-191 BANK on evidence** (the closed form supersedes the constants; the unqualified dial sentence was forbidden by R-191's own pairing) **+ 1 against the new result** (the dropped M-invariance conditioning, restored) — the expected shape when a proof supersedes a measurement, and the corpus is MORE parsimonious after the bank. P-scoring: P1 correct-as-code-test, P2 correct-and-sharpened, P3 SPLIT ON BOTH HALVES (the unbounded anti-aligned half booked as the prediction failure it is), P4 three-correct-one-vacuous, P5 correct-with-scope. FORWARD POINTER (R-193): the proven slow block is EXACTLY invariant along the drive-phase η-circle (P₀L_ηP₀ = −γP₀HP₀ at every η), and all three mobility-frame placements of R-193's computed menu reproduce THIS SAME L(k) — the linear partition is robust across the frame menu, a licensed strengthening. Records: `knowledge/audit/g8_gate2_linear_2026-08-28/` |
| R-193 | **NONLINEAR PHASE RUN N1 — THE HOMOGENEOUS-CELL ATTRACTOR: THE EXCITABLE SPIKE-LATCH, ITS EXACT ALGEBRAIC CORE, AND THE OPERATIONALIZATION MENU (pick OPEN).** In the left-mobility operationalization (op A, the flow as run), the beyond-c\* aligned-channel instability resolves as a ONE-SHOT EXCITABLE SPIKE that permanently latches the drive-phase Goldstone η: the EXACT mobility-rotation identity `L_η(k) = −(γ + c·ad_B₀)e^{−η·ad_B₀}H(k) + Ω₀·ad_B₀` — `(γ±ic) → (γ±ic)e^{∓iη}` on the ±i planes, family-wide, no ρ_sat in it (seven instruments; the reviewer's independent generator extraction 2e-5) — with the PROVEN slow block EXACTLY latch-invariant (`P₀L_ηP₀ = −γP₀HP₀` at every η, c, k — the latch lives in the ±i fast planes; extends R-192's identity). Latch permanence ANALYTIC (λ(η_f) < 0 + η neutral); six-figure closure (measured post-latch decay −0.265242 vs λ(η_f) = −0.265243 at η_f = 0.174058); endpoint ρ_sat-independent ≤ 1%; area rule quoted WITH its +16.6% error; the √(c−c\*) law STRUCK. THE LATCH WINDOW IS BOUNDED (keeper-corrected instrument `n1_tonic_map.py`): LATCH iff η_f ≥ η_stab (fired-mode entry–exit endpoint vs first full-spectrum-quiet η; failure = termination before full-spectrum stability, never runaway); at x = 14 the latch holds from c\* through c/c\* ≈ 2.7 (three instruments) and fails by c/c\* ≈ 3.3–4.4 (instrument-sensitive BOUND range; small-|k| face-diagonal breaker) — the c = 12 PRODUCTION LATCH IS REAL ON THE INFINITE LATTICE (the draft's 'c_tonic = 11.65 / cell-protected' reading WITHDRAWN as a λ_max-integrand artifact, keeper C-1 COMPUTED; endpoint stability MARGINAL in the continuum limit — soft channel O(k²)); step-deficit formula `Δ = −η_step/(T·Ω₀)` (op A, sign pinned); Γ_sat tonic arrest OPEN. **THE OPERATIONALIZATION MENU COMPUTED (banking-stopper S1 closed the C-32 way): op A / op B (gradient flow) / op C (spatial-left) all reproduce R-192's L(k) — no linear gate discriminates — and give THREE uniform-cell phenomenologies: one-shot LATCH / a RELATIVE EQUILIBRIUM with cycle-average Δ = ω/Ω₀ = +0.01301 at T = 2π/|ω| ≈ 34.5 tu (the once-quoted +1.2947e-2 is ONE CYCLE PHASE — quote the cycle-average with the period, never a point value or a band midpoint; R-194 re-cut) (the prereg's registered P4-correct/P5-failed valuable branch) **[measured on CF16 — the BAND-HOSTING cell, long-axis k_min = 0.278 = the drive-fed band peak at this dial; on band-excluding cubic cells the same dial/op has a deep-stable background (−5.78 cell-projected on C8) and no pattern forms — N2R keeper C-5; the number is the BACKGROUND's own slip, never a defect frequency]** / pattern with Δ = −1.4009e-2 (opposite sign). Pick OPEN at family-tree node V4-kernel-OP; N2 runs A and B.** Prior art credited (Adler 1946; Wieczorek et al. 2002; Kelleher et al. 2009; Mandel–Erneux/Baer–Erneux–Rinzel entry–exit rule) — the delta is exhibition on this substrate. Every η\*-derived number CELL-PROJECTED (5.4× CF16 vs continuum). ρ_sat = 0.8 early row re-diagnosed: transient-window reading, not a dt artifact. P-scoring: P1 correct (1 ulp), P2 correct-at-11-dials (5 re-established post-void), P3 correct-and-sharpened (9.03/11.14/9.12 vs 9/11.11/9), P4 FAILED and P5 CORRECT in the REGISTERED flow (op A, the flow the prereg writes out — keeper C-3; stage-D hysteresis test tautological, phase memory unmeasured; P4's failure scored with its mechanism found, no negatives row), with the op-B/C phenomenologies an UNREGISTERED finding of the round: the pattern P4 described and the deficit P5 denied, in flows the prereg did not register. RUL-114 clause 3 honoured: Δ tables are RAW data. Ceremony: contra-briefed reviewer (Opus; OVER-CLAIM — two banking-stoppers, both closed in-pass) + meta-observer (NON-GENERIC/PRIOR-ART/UNDER-CLAIMS — all enacted) + coherence keeper (COLLISION — the decisive one COMPUTED: the draft's own tonic boundary was a λ_max-integrand artifact against the meta-observer's F1 table; ten apply items executed, the correction in the result's favour) + adjudication with every decisive computation independently reproduced and persisted (the once-unscripted tonic map now committed as `n1_tonic_map.py`). **EXTENDED AT THE N2D BANK (2026-08-31, `knowledge/audit/g8_n2d_bandhost_2026-08-30/` — the row extended, not a new row, per the N2D keeper):** the op-B state is — per R-194, which SUPERSEDES-IN-PART this row's state characterisation (the exact identities all stand) — a RELATIVE EQUILIBRIUM: a frozen profile rigidly drifting along the exact B₀ gauge direction, its dial-space observable ONE number, the drift rate ω (T = 2π/|ω| ≈ 34.5 tu on CF16, transverse-size-independent on CFW16; NOT a limit cycle — the closed orbits are non-isolated gauge translates), with the cycle-average Δ = ω/Ω₀ = +0.01301 and the wind quantum the SAME number (n = 1 fixed by ρ > d, not measured — companion primitive `b0_plane_detector_wind_quantum`; the intra-cycle 'band' is ≥96% detector geometry, physical modulation ≲ 1e-5 in Δ; quote cycle-averages with the period, never a band midpoint — a biased estimator, +48% at d/ρ = 0.57). T ∝ 1/γ is an EXACT SYMMETRY of the flow (R-192's clock-scaling identity; R-193 P1 verified it to 1 ulp) — an IDENTITY, not a prediction; the un-predicted N3 content is the shape of T ACROSS the (Ω₀/γ, c/γ) plane [branch-scan re-cut 2026-08-31]. The implanted element relaxes onto the SAME relative-equilibrium family — equal collective drift rate to ≲3e-5 rad/tu with no secular divergence over ≥3 drift periods (accumulated phase difference ≤0.05 rad/100 tu against ≥0.14 free); the in-batch nulls scale linearly with their kicks; the batch elements are EXACTLY UNCOUPLED (0.000e+00 response), so this is same-attractor relaxation, NOT entrainment [R-194 re-cut; B₀-plane scope]. The winding still dies at 0.25 on the band-hosting background — (f3)(d) closed; N73's lack (ii) discharged. Defect HISTORY IMPRINTS: the op-A latch register shifts (η 0.205 vs 0.174); the op-B co-evolved element forms a different pattern configuration (polarization overlap 0.14; the once-quoted 'slip 0.0084 vs 0.0130 / 31% smaller' WITHDRAWN as a cycle-phase-and-geometry artifact — cycle-averaged −1.3%, 0.012827 vs 0.013002; the real datum is PERPENDICULAR-PLANE CONTENT d/ρ 0.570 vs 0.0083, a factor 69) — re-tiered at R-194: N2D/N3 supply NO signature on EITHER side of the §D.5.3 fork — the fading leg deflates too (the period relaxation is transverse relaxation onto the RE family, reproduced by the defect-free nulls) and the perpendicular persistence is an EXACT ZERO MODE, which is not hysteresis; what stands is the named negative: no bistability among the perturbation classes tried (leans fading, does not pick); overlap approach to 1 bounded τ ≳ 700 tu; attractor status OPEN; fenced grain-layer/one-dial/one-cell/op-B. Records: `knowledge/audit/g8_n1_homogeneous_2026-08-28/` + the branch-scan records (`g8_n2d_bandhost_2026-08-30/n2d_branchscan*`, three VERDICT files, `ADJUDICATION_BRANCHSCAN_2026-08-31.md`) |
| R-194 | **NONLINEAR PHASE RUN N3 — THE DEVELOPED op-B STATE IS A RELATIVE EQUILIBRIUM OF AN EXACT 2-TORUS GAUGE SYMMETRY, SO THE DIAL-SPACE OBSERVABLE IS ONE NUMBER (THE DRIFT RATE ω) ACROSS SEVEN DIALS; THERE IS NO MODE-LOCKING ANYWHERE, AND THE SURFACE'S SIGN STRUCTURE IS INHERITED FROM THE ROUND'S OWN LINEAR GATE.** THE SYMMETRY (DERIVED-A): the centralizer of B₀ in SO(4) is a 2-torus, generated by B₀ and ⋆B₀ ([B₀u,⋆B₀u] = 2.9e-18; exp(2πB₀u) = I to 5.5e-16), acting as EXACT gauge motions of ops B and C: step_gB(RU) = R·step_gB(U) to 2.8e-16 (in-house) — the 2-parameter form of R-193's banked left-B₀ flow symmetry, whose second generator is the branch scan's marginal ⋆B₀ coordinate (two admitted results, related for the first time; engine `g8_opB_gauge_torus_relative_equilibrium`, suite 90). THE CERTIFICATION (DERIVED-A given the committed states): the single-state residual ‖RHS(U) − ω·B₀u·U‖/‖RHS‖ returns 1e-6…8e-5 at seven of eight production dials and all three stage-B controls, against negative controls 1.000 (seeded) and 0.578 (undeveloped); 2π/|ω| reproduces every recorded period to five digits (RUL-119: the RE residual is mandatory before the word 'cycle'; C-track leg (c)). WHAT THIS SUPERSEDES IN R-193 — the op-B state characterisation ONLY: 'a LIMIT CYCLE' is MISLABEL (the trajectory closes exactly, but the closed orbits are NON-ISOLATED: the ⋆B₀-translate is a distinct closed orbit of identical period, O(1) away — min over the drift circle 0.62–0.74 against a translate residual 1.8e-14); the state is a ROTATING WAVE, steady in the co-rotating frame and in every gauge-invariant observable; T ≡ 2π/ω and Δ_cycle ≡ ω/Ω₀ are ONE measurement, and 'wind quantum n = 1' is fixed by ρ > d (detector clause (i)) — geometry, not a measured integer. R-193's exact identities (mobility rotation, slow-block latch invariance, the op-A latch, the equivariance dichotomy) are UNTOUCHED. THE ω-SURFACE (MEASURED, RAW): seven dials — (10,8) 0.044815 / (10,10) 0.079840 / (12,10) 0.147878 / (12,12) 0.090158 / (14,12) 0.182084 / (16,15) 0.149727 / (20,18) 0.165092 rad/tu (T = 140.20/78.70/42.49/69.69/34.51/41.96/38.06 tu); non-monotone, dΔ/dc flipping sign between Ω₀ = 10 and 12; estimator sd ≤ 1e-4 tu. THE INHERITANCE (FIT, counted): ω ≈ 0.244·λ_cell^0.466 (10.3% max resid, n = 6) or 0.280·λ_cell^0.613 (44.5%, n = 7) — supercritical-amplitude scaling on the round's OWN design-gate rate; all three legs of the non-monotonicity reproduced IN SIGN; ω tracks the CELL's rate (rel sd 16.7%) not the continuum peak (32.1%), and CF16's hosting deficit is 0.8/3.3/47/31% at the four first-certified dials — the 'first genuinely un-predicted' framing is STRUCK; what stands un-inherited is the ±2–10% residual at 10³–10⁴σ. THE LOCKING NULL (MEASURED — the round's most durable output): T/T_drive = 81.1/76.9/106.9/121.1 — no low-order rational, no plateau, NO LOCKING at any certified dial; and structurally so: the flow is autonomous and Ω₀ enters as a rate along the gauge direction, so the well-posed question is the DRIFT BIFURCATION (ω = 0 vs ω ≠ 0; ω → 0 at threshold, Im λ = 0 at every certified dial — Rand/Krupa behaviour); the registered Arnold-tongue question Q2 is answered NEGATIVELY. THE ⋆B₀-KICK STOP (DERIVED-A): the kick is an exact gauge motion — after 80 tu of independent integration the kicked endpoint equals R_kick × the cont endpoint to 1.8e-14; the |a_k| doubling is the loss of the observable's DC Fourier component (modulus folding, RUL-118: fixed-projection trackers are NOT clocks; extremum spacings predicted at zero free parameters); registered expectation confirmed at machine precision. ρ_sat (MEASURED; Q3 OPEN): the ceiling was never engaged (δg ≤ 2.3%; ΔT ratio 105.0 vs the code's own 100.0); dlnT/dδg ≈ −0.25 over two decades; the ~25%-at-engagement figure is an EXTRAPOLATION; probing regime ρ_sat ≲ |F| ≈ 0.45. PRIOR ART (credited; the delta is exhibition on this substrate): Adler 1946; Rand ARMA 1982; Krupa SIAM JMA 1990; Cross–Hohenberg RMP 1993; Aranson–Kramer RMP 2002; Kramer–Zimmermann 1985; Coullet–Emilsson 1992; Petrov–Ouyang–Swinney 1997. FENCES: CANDIDATE kernel · GRAIN layer · op B (op C untested; NOT RE-/CYCLE-/GEOMETRY-CHECKED) · CF16, one hosted wavevector k = (−0.1963, 0.1963, 0, 0) at every dial of the 5×6 grid · C-N second-cell check executed at (14,12) + (16,15): ω identical across cells to ±0.003% at BOTH dials incl. the 47%-deficit one — transverse-robust at fixed hosted mode; the cell-vs-continuum k question stays open (`n3_cn.py`) · γ = 1 · B₀-plane detector scope, (ρ, d) shipped, VOID if ρ ≤ d · cycle-averages only (RUL-115) · RUL-114 clause 3: raw dynamical data, no mass/generation/ladder/count reading · ★ ω is the uniform BACKGROUND's collective B₀-plane drift rate — not a defect frequency, not a meta-time rotor frequency; no layer-crossing to mass = ω is licensed (L2-1, EOM-map §5 assembly record). Ceremony: contra-briefed reviewer (the RE finding) + meta-observer (the hosting confound, all computed) + keeper ×2 (pass 2: 5 collisions all computed, the largest resolving AGAINST a same-day label in favour of a prior-day identity) + adjudication with every decisive computation reproduced in-house (`n3_inhouse.py`). Records: `knowledge/audit/g8_n3_tongues_2026-08-31/` |
...
| R-183 | **★ THE STERILE CROSS-BRACE — the sterile partner is pinned from TWO INDEPENDENT DIRECTIONS, and each leg is the other's failure mode.** **LEG 1 — IT MUST EXIST**, via the bosonic-lattice license: I-31's repaired `I-SMG-4` distinction is **fermionic-lattice versus bosonic/qubit-lattice** (not "extra structure"), so the `15n` (no-`ν_R`) escape that a *fermionic* lattice could take **is not available to a bosonic substrate** — this family's per-generation content must complete to the anomaly-free **15 gauged Weyl + 1 sterile = 16**. **LEG 2 — IT MUST BE EXACTLY HYPERCHARGE-NEUTRAL**, via R-159's charged-`ν_R` under-determination control: give `ν_R` a hypercharge and the two-branch forcing of the continuous anomaly system **dissolves**, taking `Q_p + Q_e = 0`'s anchor-free character with it. **MODUS TOLLENS BOTH WAYS, which is the row's whole content:** *drop* the dynamical sterile → the bosonic-lattice license is forfeited and the `16` is unmotivated; *charge* it → R-159's forcing is forfeited. Neither leg is reachable from the other — one is a lattice-regularization statement about the grain layer, the other an exact algebraic control inside the charge sector | **CANDIDATE, TIERED AT THE WEAKER LEG.** Leg 2 is **engine-exact** (`charge_normalization_anchor_free`, re-verified at public HEAD by the round-4 commission reviewer). Leg 1 is **CONJECTURE-CONDITIONED**: it rides I-31's SMG line, whose quarantines travel with it — Eichten–Preskill and the 3+1D constructive-evidence line are **unread and `[SNIP]`-quarantined**, Wen's statement is a **construction claim, not a demonstration**, and Golterman–Shamir remain live opposition. **A brace is only as strong as its weaker leg, so the joint claim is conjecture-conditioned**, and the favourable coincidence of content is **never to be quoted as support** (I-31's standing fence). RUL-049 conditioning travels with every restatement | — (**no primitive; nothing new is computed** — the row is a *joint reading* of `charge_normalization_anchor_free` (leg 2) and I-31's `I-SMG-4`/`I-SMG-5` sub-rows (leg 1)) | C.2.7, C.5.4, E.1 | R-121, R-159, I-31 (`I-SMG-4`, `I-SMG-5`) | — | ★ **BANKED-PENDING-§8a** — proposed, not adversarially reviewed; do not cite as settled. **PROVENANCE:** proposed by the **round-4 external (Fable-class commission) reviewer** in his warm response, digested at `knowledge/audit/external_review_r4_2026-08-24/WARM_RESPONSES_DIGEST_2026-08-24.md` §B.2 (disposition: DOCKETED §8a banking candidate); **entered by this batch's banking pass**, report at `.../FOLLOWUP_BATCH_REPORT_2026-08-24.md`. **REVERT CLAUSE:** if I-31's bosonic-vs-fermionic distinction is withdrawn at primary, **leg 1 falls and the brace collapses to R-159's leg alone** (which stands independently); if R-159's premises P4–P7 are disturbed, leg 2 falls and leg 1 stands alone as an unbraced conjecture. **Striking the row costs nothing downstream** — nothing consumes it; what is lost is the record that two unrelated lines of the corpus constrain the same object from opposite sides |
...
| R-087 | B − L anomaly cancellation from `3 × 1/3 = 1` — **rescoped 2026-08-24 (OI-1 §8a round): the headline sum is the DOUBLET-SECTOR condition, no longer billed "anomaly-free" outright**; the singlet-inclusive `Σ(B−L)` and `Σ(B−L)³` are each `−1` on the gauged 15 and complete to 0 exactly with R-121's sterile (classical completion, no SMG import; the sums are colour-multiplicity-insensitive — they consume the state pattern) | DERIVED-A | B_minus_L_anomaly + anomaly | C.5 | R-057, R-121 (the completion) | R-089 | One quark of charge 1/3, three colours; one lepton of charge 1. Full free-`c` anomaly system: R-159 (`charge_normalization_anchor_free`); record `oi1_anomaly_ledger_2026-08-24/`. |
...
| R-091b | Nuclear length hierarchy: hard core `√2 ℓ_S ≈ 0.397 fm` vs the customary 0.40–0.50 fm range (no new parameter — **0.8% below the near edge, 21% below the far edge; the old midpoint-referenced "12%" is retired, since a range this wide discriminates little and its referent is a feature of particular NN-potential parametrizations rather than a measured length**); pion Yukawa `~1.46 fm`; 25-cell footprint (1 + 24 = D4 kissing); `r_{90} ≈ 0.518 fm`, soliton diameter `~ 1.12 fm` | DERIVED-given-(e, f_π) — no NEW parameter (ℓ_S = ℏc/(e·f_π) rides the two ANW-fitted constants) | nuclear_length_hierarchy + skyrme_length_fm (cover the four lengths; the r_90/25-cell prose is not engine-covered) | C.5 | R-051, R-053 | — | Cell-exclusion phenomena set both hard core and confining-string diameter. 25-cell structure ties directly to A-1b D4 kissing. |
...
| R-121 | Three sterile right-handed neutrinos as parameter-free DERIVED prediction (B−L conservation forces Dirac character; RH partner is `S_−`) | DERIVED | sterile_rh_relic_check | E.1 | R-089, R-076 | R-122, **R-183** | Structural; engine-banked. **★ CROSS-BRACE (R-183, BANKED-PENDING-§8a):** the sterile partner this row predicts is *also* constrained from two unrelated directions — it must EXIST (I-31's bosonic-lattice license) and must be EXACTLY hypercharge-neutral (R-159's charged-`ν_R` control). Read R-183 before quoting this row as independently secured: the brace is tiered at its **weaker, conjecture-conditioned** leg. |
| R-122 | Sterile RH relic: sterile SHARE ~1.1% of Ω_DM, a **94×–176× shortfall depending on which cosmological `Σm_ν` bound is taken** (94× at the banked Planck 2018 + BAO 0.12 eV input; ~176× under DESI DR2's ΛCDM 0.064 eV bound, PRD 112, 083515 (2025); ~70× under its w₀wₐ 0.16 eV bound — the chain is linear in `Σm_ν`, the ΛCDM leg is the adverse one, and no site may restate the figure without naming the model) (the active+sterile TOTAL is ~2.1%/47×, or ~88× at the tighter bound — the engine key predating the relabel reports the total; E-6, 2026-07-31); the remaining ~98% is now the inter-front programme's TARGET (handoff §7, 2026-07-31), not a standing scope fence | LOCATED-GAP | sterile_rh_relic_check + sterile_rh_z2_separate_mass_scale_check + sterile_rh_substrate_production_via_L_theta | E.1 | R-121 | — | Canonical falsifier §E.3 VG-4. Z1/Z2 LOCATED-GAP-REFINED; Z3 still OPEN. Lead (i) differential coupling and (ii) wave-train phase-defect remain OPEN. |
...
- *Named near-term* (16 rows, in table order): `c_GW = c_γ` (R-039), proton decay (R-054),
  ...
  **Retired 2026-07-27:** the two former head rows — UHE-CR LV and `α_3` — were DELETED, not
  ...
  — E21-split 2026-08-26: ~1 order unconditionally, 6–7 only under disfavoured pure-proton
  composition; the 3–9 range is RETIRED with the wide bracket (which-Λ ruled 2026-07-30: naive
  `η⁽⁴⁾ ∈ [1.9, 6.7]` on the `Λ_L = 1/a` band — see R-037) — so the item is
...
- **R-165a** (no anisotropic quartic ⇒ leading rotational anisotropy at **dimension eight**) ← R-004
  ...
  (P-an, P-pg, P-op)** ride this edge, not the node: analyticity in `k`, the FULL group including
  triality, and **(P-op)** that the symmetry operative on the *spatial* anisotropy sector is that
  full group `[1152]` rather than the driven subgroup `Stab_G(e₄)` `[48]` — at which the degree-4
  spatial invariant space is 2-dimensional and `Σ_{i≤3}k_i⁴` is permitted (R-165 row, 2026-08-25).
  Substrate-specific, not generic — `Z⁴`'s point group admits `Σk_i⁴`.
...
- **R-175** (KC-1, the one-way symmetry-class filter) ← `canting_vacuum_branch_structure` + R-174 (the same Bloch stiffness rig) + I-29 (the paraunitary class named as the one that CAN pass). *Outbound:* **R-191** (`g8_kernel_kc1_gate`, 2026-08-28 — the first kernel run through the filter: G-8 gets PAST the necessary condition, so the filter's verdict on it is NOT DECISIVE); the other consequential outbound remains the **negative on TWT's own R-112 hyperbolic linear face**, which fails the filter identically (`Ω` absent, `ω² ∝ eig H`). The formerly-unbuilt inbound condition is **closed**: `M` exhibited (R-191), the real-class assignment for `H` an exact identity.
...
- **R-165a** dimension-eight anisotropy: the edge carries premise **(P-an)**, analyticity of the
  ...
  A fourth premise, added 2026-08-25 and structurally the same hazard shape as (P-an): **(P-op)**,
  that the operative symmetry on the *spatial* anisotropy sector is the full point group and not
  the **driven** subgroup the drive actually leaves intact. The theorem is proved in four Euclidean
  variables at `[1152]`; the claim is quoted for three spatial ones, and `Stab_G(e₄)` `[48]`
  restricts faithfully onto `W(B₃)`, whose degree-4 invariant space is 2-dimensional and contains
  `Σ_{i≤3}k_i⁴`. The same reversal applies: a Layer-1 invariant-theory node is conditional on a
  Layer-2 object (which symmetry the driven steady state respects). At the driven group the
  protection rests on the bond set's computed zero spatial fourth moment — interaction content, not
  symmetry. See the R-165 row for the computation and the would-change-if.
...
*Total primitives in the engine: 396 as of 2026-08-31 (three companion additions at the 2026-08-31 banks: `b0_plane_detector_wind_quantum`, `two_defect_tensor_complex_space`, `g8_opB_gauge_torus_relative_equilibrium`; previously 393 as of 2026-08-27 (FOUR files since the 2026-08-23 family split — the MAIN engine is `twt.py`, a pure import FACADE defining nothing, over two halves: `twt_core.py` CORE (family-level) and `twt_candidate_v3.py` CANDIDATE (V3-instance). MAIN: 316 module-level defs = 280 public + 29 helpers prefixed `_` + 10 classes, split 205 CORE (174 public + 21 helpers + 10 classes) + 114 CANDIDATE (106 public + 8 helpers); `twt_companion.py` COMPANION: 80 defs = 67 public + 13 helpers, carrying the same cut as an in-file SECTION split — 30 defs in SECTION CORE (22 public) and 50 in SECTION CANDIDATE (45 public); the three 2026-08-31 additions are two_defect_tensor_complex_space (CORE) + b0_plane_detector_wind_quantum and g8_opB_gauge_torus_relative_equilibrium (CANDIDATE). The split MOVED code and rewrote nothing: the only non-move edits are the two the ruling mandated — the `CORE_PROVENANCE` registry and R-161's witness split. Re-counted by AST 2026-08-27, not incremented. Refresh this count+date at each banking pass, and count rather than increment). The table below lists the
267 load-bearing-primitive rows (re-counted 2026-08-28, not incremented; 267 distinct primitives —
the second, byte-identical `B_minus_L_anomaly` row was removed at the 2026-08-18 archivist
pass, so rows and primitives agree; `weak_su2_menu_exhaustion`, `charge_assignment_from_anchor` and `charge_sector_provenance` added 2026-08-21; `weak_host_must_be_body_frame` and `lock_left_centralizer_is_u1` added 2026-08-23 (R-172); `brannen_comb_commitment_dominance_and_dof_vacuity`, `magnon_stiffness_bands_canted_vacuum` and `spectral_branch_symmetry_class_filter` added 2026-08-23 (the estate of N64, R-173/R-174/R-175); `spinor_module_graded_iso` added 2026-08-23 (R-182, the V4-ASD Layer-A round's only bankable result); and the six-primitive Γ-channel referent closure `bond_invariant_menu_frame_bilinear`, `bond_channel_parity_exclusivity`, `bond_harmonic_ceiling_by_generator_class`, `gamma_survivor_pitch_genericity`, `DoverJ_calibration_referent`, `gamma_admixture_cross_functional_route` added 2026-08-23 (R-176…R-181); `bilinear_detection_third_order_null` added 2026-08-25 (R-184, the Sorkin arc's bank); `driven_group_spatial_invariants_wb3` and `d4_scalar_dispersion_quartic_coefficients` added 2026-08-25 (R-185/R-186, the (P-op) executable content and the external-proposed dispersion split); `koide_branch_restriction` added 2026-08-26 (R-187, the external-proposed Koide branch domain); `core_healing_length_canted_vacuum` added 2026-08-27 (R-189, PROBE B's tree-level healing length and, from its own contra-review, the N70 reference-state instability); `marginal_quartic_running_reach_to_cell_scale` added 2026-08-27 (R-190, PROBE C's I-13-free one-loop rate `−1/(6π²)`, the chiral-counting exactness theorem, and the IR-anchored reach of 1.9937 e-folds against 36.68–42.00 — the mechanism applies and does not reach); `g8_kernel_kc1_gate` added 2026-08-28 (R-191, kernel programme gate 1 — KC-1 not decisive for G-8, the first kernel past the necessary condition, with KC-1's soft spot closed in the same pass); `g8_gate2_linear_partition` added 2026-08-28 (R-192, gate 2 linear — the exactly-2D dial partition, the proven slow block superseding R-191's constants in closed form, and the selection strip); `b0_plane_detector_wind_quantum` (COMPANION) added 2026-08-31 (the branch-scan bank — the B₀-plane detector's off-origin-circle law, winding transition and wind quantum; cited by the R-193 row and R-194); `g8_opB_gauge_torus_relative_equilibrium` (COMPANION) added 2026-08-31 (R-194 — the exact 2-torus centralizer, the modulus-folding wall RUL-118, the RE residual RUL-119)) that V3 R-NNNs cite
...
| bilinear_detection_third_order_null | B.3.3 | R-184 | twt_test.py |
| charge_normalization_anchor_free | C.2.7, C.2.8 | R-159 | twt_test.py (this primitive also banks the continuous hypercharge ANOMALY system — the TWO-BRANCH forcing, the Witten doublet parity, the charged-ν_R control; searchable terms carried here per the OI-1 findability repair) |
...
| driven_group_spatial_invariants_wb3 | B.1.5 (premise register, the (P-op) row) | R-185 | twt_test.py |
| d4_scalar_dispersion_quartic_coefficients | B.1.5-adjacent (Core §5.3) | R-186 | twt_test.py |
| koide_branch_restriction | C.3.2–C.3.4 | R-187 | twt_test.py |
| charge_orbit_boost_invariance | C.2.7-adjacent | R-188 | twt_test.py |
| core_healing_length_canted_vacuum | D.4.3 / D.5-adjacent | R-189 | twt_test.py |
| marginal_quartic_running_reach_to_cell_scale | C.5.2 / D.5 | R-190 | twt_test.py |
...
| g8_kernel_kc1_gate | D.5 | R-191 | twt_test.py |
| g8_gate2_linear_partition | D.5 | R-192 | twt_test.py |
| g8_n1_lock_phase_rotation | D.5 | R-193 | twt_test.py |
...
- `two_defect_tensor_complex_space` (COMPANION CORE, 2026-08-31): the pointwise two-defect complexification — all 9 winding-blade pairs give exactly ℂ²⊗ℂ², singlet slot choice-invariant; SCOPE in-code (pointwise only; bundle-level Hopf question untouched; N53 stands). No R-row by keeper ruling; N53 route-(c) annotation is its ledger site.
...
- R-184: `bilinear_detection_third_order_null` — the general-bilinear third-order null; an independent non-Gleason route to R-160's CONCLUSION (not its chain), one-way and needing the empirical third-order null adjoined
...
- R-185: `driven_group_spatial_invariants_wb3` — the (P-op) premise's content made executable: `Stab_G(+e₄)` ≅ `W(B₃)` faithfully-exactly, spatial degree-4 invariant dim 2 (two independent methods), full-orbit spatial isotropy with the ±4 split as the e₄-axis sensitivity decomposition; NOTHING MOVES, computed ≠ discharged, the §D.5.7 Γ-survivor arbitration stays OPEN
- R-189: `core_healing_length_canted_vacuum` — PROBE B (frozen pre-registration, full ceremony): the tree-level linearised healing length per gapped direction per branch, `ξ/a` from 5.39 to ~30 with the naive `√(12/g)` shown to be the FLOOR not the value; and, from the contra-briefed §8a review of the probe, N70 — neither computed vacuum branch is a tree-level local minimum, so branch selection is fully kernel-routed. One reviewer sub-claim pushed back and withdrawn; one novelty over-claim of our own (the `±k₀` points, already banked at KC-1) self-caught at bank time and withdrawn
- R-190: `marginal_quartic_running_reach_to_cell_scale` — PROBE C (frozen pre-registration, full ceremony), the ★★★ docket item: the sector's only log-runner has an **I-13-free** one-loop rate `β_λS = −1/(6π²)` whose sign independently corroborates R-148, and a chiral-counting **exactness theorem** (no power of the quartic couplings at any order — stronger than QCD's own β). But the flow is **IR-free, hence IR-anchored**: its free parameter sits at the *measured* hadronic end, and the reach is **1.9937 e-folds against 36.68–42.00** required. **THE MECHANISM APPLIES AND DOES NOT REACH** — the first build's "the mechanism does not apply" and "no coupling value works" were both refuted by the contra-briefed review and struck; a vacuous third "control" (γ₂ cancels identically) was caught and is now **asserted as a vacuity demonstration** so it cannot return. "Earned dimensionful scales" stays at **0**
- R-186: `d4_scalar_dispersion_quartic_coefficients` — the external-reviewer-proposed scalar dispersion split: both quartic coefficients vanish at orbit-constant coupling (unique root), ~10⁻² at the V3-2a sub-orbit extreme; model rider mandatory
- R-187: `koide_branch_restriction` — K = 2/3 is branch-restricted (a quarter of the phase circle, edges at −1/√2); the physical phase sits 0.0396 rad inside the edge (noted non-coincidence); second externally-proposed bank
- R-188: `charge_orbit_boost_invariance` — the charge orbit is exactly boost-invariant (leakage 0.0), commissioned by an external objection that predicted failure; the cost clause (bought by holding e₄ fixed) is mandatory in every restatement
...
versioned worker-formation prefix) is in the `apparatus/` directory of
**https://github.com/yaerhf/TWT**, beside the engine, as it
...
the `Λ_L = 1/a` band, retiring the wide bracket. Then 2026-08-26 the E21-VERDICT re-cut the
EXCLUSION itself on primary reads: ~1 order unconditional / 6–7 conditional on pure-proton, the
3–9 range retired — the figure that survived four review rounds because it counted against the
programme, the adverse-claims sweep's founding case.)*
Escalated
...
### G.16 PROBE C: the hierarchy question answered, and two corrections against ourselves (2026-08-27)

**What was asked.** The ★★★ docket item, and the first thing in the programme that could have moved
*"earned dimensionful scales: 0"* off zero: does the marginal Skyrme quartic's running carry the
tree-level core scale up to the hadronic cell scale by dimensional transmutation, and what
microscopic coupling would that demand? The pre-registration was frozen and committed **alone**,
before any script existed, and PROBE B ran first so its banked numbers were the named input.

**What came back.** No — and the honest form of the answer is **"the mechanism applies and does not
reach"**. The flow is one-loop exact and infrared-free, so its free parameter sits at the *measured*
hadronic end: the reach is `λ_S(cell)/c = 1.9937` e-folds against the `36.68–42.00` required. The
result banks as R-190 and N71, and **the Core paper's scale-class row does not move.**

**The first correction, and it is the one worth recording.** The probe's first build gave the wrong
*reason* for the right answer. It argued that the β-function is additive, therefore has no pole,
therefore the transmutation formula does not apply at all. The contra-briefed review refuted that by
computation and the refutation was independently re-verified before it was accepted: *every*
one-loop β is additive in the inverse coupling — QCD's included — and the probe's own positive
control, recomputed as an additive e-fold count, returns the identical number it had computed. The
probe had validated its pipeline with the very formula it declared inapplicable. A second sentence
fell with it: *"no coupling value works."* One does, and it lands inside the pre-registration's own
target window; what excludes it is a 21× conflict with the framework's banked coupling, a 95×
conflict with the measured one, and its lying ~25× outside the domain of the β that would carry it.
**The block survived the loss of its stated ground and came out stronger**, because two of the
review's findings were under-claim repairs.

**The second correction is a vacuous check, caught at the exact signature the canon names.** The
probe had filed a third "control" claiming that an RG-invariant landing on the chiral scale to 3 %
corroborated *the magnitude* of the computed rate. The rate cancels identically out of that
quantity — verified at four values, one wrong by a factor of 700, all returning the same number to
sixteen digits. *A tight tolerance on a quantity that cancels is a tell, not rigour.* The claim is
withdrawn at every site, the cancellation is now **asserted in the suite** so it cannot return, and
the arc's negative control was separately found tautological. What replaced the three-way "PASS" is
a plain sentence: **run-validity rests on the positive control alone.**

**Process note.** Sixteen findings, thirteen accepted outright, two partial pushbacks — both
adjudicated accepted, and both in the *conservative* direction, one of them claiming less than the
review offered because the review's own findings were in tension with each other. A misfit was also
recorded against the frozen pre-registration itself: its outcome classes assumed a UV-anchored,
asymptotically-free sector with a microscopic dial, and this sector has neither, so by the letter of
those classes the required coupling lands in the "generated" window. Saying so is what stops a
reader reconstructing the wrong outcome from the probe's own numbers.

### G.15 The priced comparison enters the Core paper's §3 (2026-08-27)

**Where it came from.** An **external** proposal, and the credit belongs there: the round-5
external reviewer drafted a paper-ready subsection (`knowledge/audit/external_review_r5_2026-08-25/`
`review_r5_07_consolidated.md` §7.1) and named its intended home himself — a new block of the Core
paper's §3, after the separator's application and before *"Three things this section does not
do."* Adopted on the human coordinator's call. It answers a **measured** problem rather than a
preference: the A/B run traced a test reader's adverse initial posture to his concluding that the
comparative ledger was unpublished, at a moment when the restructure had just moved the
comparative material out of the paper in favour of a pointer. A pointer only helps a reader who
follows it, so the accounting is now in the paper at ~1,020 words against the 6,084 the
restructure saved.

**What was adopted, and what was changed on the way in.** The draft entered essentially intact —
its register is the point of it, and every claim was verified against the live corpus before
insertion rather than carried on the draft's word. Changes: the §-references were re-keyed to the
live Core paper (§5.2's no-summing fence, §2.1's flat-direction closure, §2's conditional
strengths, §5.3's two wounds, §4.4's exposure); the draft's *"this family's dimension-six
exposure"* was corrected to **this candidate's** dimension-six ceiling, which is §5.3's wound and
not the family-level colour exposure; the ceiling is described as *a constraint already in force*
with **no order-of-magnitude figure**, the E21-re-cut conditioned split staying where it lives; the
inventory sentence was re-keyed to the live §5.1/§5.2 wording (*fourteen recorded choices*, and
the *largest structural inventory in the comparison* quoted no stronger than §5.2's row, whose
SURVEYED rider on the incumbent's column governs it); and the draft's closing fence was rewritten
to stop restating the parsimony sentence that stands two lines below it in *"Three things this
section does not do"* — the concession now hands off to that paragraph instead of twinning it. The
third criterion clause carries the draft's *named and unblocked route* wording without the
*not effort spent* gloss, which would have read against §3's own earlier *being worked*.

**Verified, not assumed:** the ledger-level findings the closing pointer names — 5, 20, 22, 34 and
41 — were each read in `TWT_COMPARATIVE_LEDGER.md` and are genuinely the load-bearing adverse
ones (`DOUBLE-BILLED`, the zero-earned-scales charge, the three-coupled-premises repair, the
abstention charge, and the ten-shrinks-to-two credit over-claim). The incumbent-facing numbers
agree word-for-word with §5.2's rows (19; 26–28 with neutrinos) and with §4.4's credit of *the one
earned dimensionful scale in the comparison, the strong scale by dimensional transmutation*.

**Deliberately excluded.** The reviewer's own §7.2 warning is honoured rather than imported: the
*"eyes of a physicist from 200 years ago"* device stays out, because it works by removing the
evidence that made the incumbent's posits earned and a sharp referee identifies that in one line.
The adverse scorecard is excluded by the coordinator's explicit call. And the reviewer's ordering
constraint is structural, not stylistic — the incumbent's **earnings** first, its unnamed **debts**
second, this family's **price** third, the **concession** last; his note that *"if a later edit
moves the concession out of final position, the section stops working"* binds every future edit to
that block.

**One roster line moved with it:** *How to read this paper* now says §3 is the separator applied to
the family's own list **and the comparison with the incumbent priced**. The paper itself stays
history-blind — none of the above is in it.
...
- **LIGO / Virgo Collaboration** (2017). *Gravitational waves and gamma-rays from a binary
  ...
  §4.1 gives the two-sided asymmetric interval `−3 × 10⁻¹⁵ ≤ Δv/v_EM ≤ +7 × 10⁻¹⁶`, conditional on
  the assumed gamma-ray/GW emission-time offset and on the conservative `D = 26 Mpc`; the flat
  `≲ 10⁻¹⁵` shorthand is order-of-magnitude only and must not be quoted as a threshold. §E.3 row 1,
  §E.3 VG-1.
- **Planck Collaboration** (2020). *Planck 2018 results: cosmological parameters*. Astron.
  Astrophys. **641**, A6. — `Ω_k`, `Ω_DM`, `Σ m_ν < 0.12 eV` (95% CL, ΛCDM); §A.2, §E.1.3.
- **DESI Collaboration** (2025). *DESI DR2 Results II: Measurements of Baryon Acoustic
  Oscillations and Cosmological Constraints*. Phys. Rev. D **112**, 083515. —
  `Σ m_ν < 0.064 eV` assuming ΛCDM (95% CL, DESI BAO + CMB) **but `< 0.16 eV` in the w₀wₐ
  model**: the neutrino-mass bound is model-split and the corpus quotes it as a range with the
  model named. The tighter ΛCDM leg roughly doubles §E.1.3's sterile relic shortfall, i.e. it
  runs against the framework. §E.1.3, §E.3.3 VG-4.
- **Goswami, S., Chakravarty, G. K., Mohanty, S. & Prasanna, A. R.** (2017). *Constraints on
  cosmological viscosity and self interacting dark matter from gravitational wave observations*.
  Phys. Rev. D **95**, 103509. — `η < 2.3 × 10⁹ Pa·s` for the cosmological fluid, from GW150914
  over 410 Mpc: a published bound of the same form as §E.3.3 VG-1's own GW-propagation ceiling
  and roughly 2.8× tighter than the corpus's reproduced `6.5 × 10⁹`. Cited as an empirical
  witness, not imported as a theorem, so no Section-13 registry row is owed; the channel match
  (stress tensor, shear, cell layer, GW band) is stated rather than assumed — their `η` is the
  cosmological fluid's and the corpus's is the substrate's. §E.3.3 VG-1.
...
| Isotropic dim-6 LV coefficient `η⁽⁴⁾` (R-165 / VG-6 / N52) | **2** | Blocked on the #1-gap kernel's strain-mode dispersion. Emphatically NOT Class 4: knowability is precisely what does *not* limit it — the number is already measured; what is missing is the substrate computation to compare against it | Four named handles (N52): a symmetry forcing the isotropic quartic coefficient to vanish; `Λ_LV` decoupled from the Sakharov cutoff; form-factor suppression sized to the CONDITIONING (E21-split 2026-08-26: ~1 order to clear the unconditional Auger bound — which the banked `(f_π/m_p)² ~ 10⁻²` arithmetically covers, applicability unbuilt — vs 6–7 orders for the pure-proton-conditional corner; which-Λ ruled 2026-07-30, `Λ_L = 1/a ∈ [0.39, 0.73] M_Pl`, naive `η⁽⁴⁾ ∈ [1.9, 6.7]` — R-037) *plus* a photon-sector account for the conditional corner only (the photon `10⁻⁸` is PROJECTED, trigger unmet); heavy UHECR composition — the handle E21 PROMOTED: it is not a 1–2-order trimming but the difference between the conditional corners and the ~1-order unconditional bound (the Fe branch straddles the naive band) |
...
| I-6 | **SM renormalization-group run-down** (gauge-coupling running between the unification and measured scales) | §C.4.5 — the `3/8 → M_Z` run-down (the earlier "→ 0.231" reading is **RETRACTED**: with TWT's own SM content the run-down from the ruled `Λ_L` lands at `0.154–0.158` — before the uncomputed lattice→continuum matching premise, §C.4.5; `0.147–0.164` at the retired wide bracket — and reproducing the measured `0.23122` needs an underived `M_X ≈ 1.09 × 10¹³ GeV`), Λ_QCD comparisons (R-133 knock-on). **EXTENDED 2026-07-29 to two loops** (Machacek–Vaughn SM gauge beta functions plus the top Yukawa) for the N55 escape-route closure — same import one order higher, but note it adds two premises the one-loop use did not carry: the **MS-bar scheme** (two-loop betas are scheme-dependent) and the `y_t(M_Z)` input. Both were tested: the two-loop shift is `+0.0004`, i.e. 0.6% of the gap, and is stable to `~10⁻⁵` against ±10% in `y_t` and against dropping the Yukawa entirely; the absolute band carries a `~10⁻⁴` systematic from `α_em(M_Z)` and omitted weak-scale matching, so it is not to be read past the fourth decimal | Inside-frame data bridge | Full SM field content; perturbative unitarity over the run; **AND — premise added 2026-08-24 (round-4 commission item 1, finding F-2, adjudicated PART-RIGHT) — the validity of *elementary-field* renormalization-group equations for a gauge sector this candidate holds EMERGENT / composite at `Λ_L`.** Near compositeness the two-point function develops form factors and "running" is not defined, so what rides this premise is the import's *applicability*, not merely its accuracy. **Conditional consequence, recorded with it: if this premise fails, R-082's descent-window reading (the retracted "→ 0.231" claim — the round-4 merge record's shorthand "R-082b"; there is no separate row of that ID) reverts from REFUTED-as-reading to GATED — no computable descent at all.** *(F-2's base claim that the run-down is "an import nowhere registered" is FALSE: this row is the registration, and the two-loop extension for the N55 closure is recorded in it. What F-2 correctly located is an incomplete premise set, not an unregistered import — so this is an amendment, not a new row, and O9(b)/F11 stay refuted.)* | **OPEN as derivation, SANCTIONED as data import** — canon §7 states plainly TWT does not derive the running; this is the inside-for-data method, not a disguise | TWT-native running (P2-3's β and the P2-1 kernel) — the registry's largest single exposure by headline-value. **Second handle, added 2026-08-24 with the premise above:** exhibit the emergent gauge propagator's form factor from the kernel — which coincides with the P2-3 handle and discharges both at once |
...
| I-13 | **The dispersive/S-matrix package** (analyticity, crossing, ≤2 subtractions/boundedness, optical-theorem positivity — the KL+unitarity route R-085 named; FIRED 2026-07-05) | R-148, §C.5.2 — the β₃ sign (AF-signed at the dressed level; the wrong-sign risk removed) | Inside-frame / effective (elastic ππ-class forward channel of the dressed coset Goldstones) | Analyticity/causality of the inside-frame amplitude; crossing; polynomial boundedness (≤2 subtractions); optical-theorem positivity | **DATA-LIKE jurisdiction (13.4)**; the positivity leg PARTIALLY RECAST onto the banked §B.3-derived unitary QM (probability conservation of the derived theory — the 13.3 stability-recast directive satisfied for that leg); analyticity + boundedness NOT yet recast (named residual premises) | Retired by the kernel's own UV spectral computation (the same object that owes DGLAP). **REVERT CLAUSE, NARROWED 2026-08-27 (keeper C-2; the blast radius SHRINKS, which is §13.4's excisability discipline working):** refusing the package reverts R-148's **dispersive derivation**; the **sign proposition SURVIVES** on R-190's chiral one-loop route (import **I-32**, call-closure disjoint from R-148's — AST-verified) **within its stated domain `μ ≲ Λ_χ`** (R-190 leg L-D). So what a revert restores is the located gap **ABOVE that domain**, not the sign itself. The former wording — *"R-085's located-gap status (β₃ sign genuinely open)"* — was written 2026-07-05 when only one route existed and is **withdrawn as over-broad**. R-148 still reverts to the pre-import state; nothing else moves. **Used-at unchanged (R-148, §C.5.2): R-190 does NOT use this import** |
| I-32 | **Gasser–Leutwyler one-loop chiral running of the `O(p⁴)` low-energy constants** — the anomalous dimensions `γᵢ` in `lᵢʳ(μ) = (γᵢ/(32π²))(l̄ᵢ + ln(M²/μ²))`, hence `μ dlᵢʳ/dμ = −γᵢ/(16π²)`. **The VALUE `γ₂ = 2/3` (and `γ₁ = 1/3`) is READ, not computed** — Gasser & Leutwyler, *Annals of Physics* **158** (1984) 142, read at primary this session via arXiv:2004.03992 (a positivity-bounds paper, i.e. I-13's own literature family), which quotes the basis, the renormalization formula and the `γᵢ` verbatim. **Only the FUNCTIONAL FORM is derived in-suite** (Weinberg chiral counting: `β_{l₂}` carries no power of the quartic couplings at any order — exactly additive, exactly one-loop). Registered 2026-08-27 in the same pass as R-190, per canon §2 | **R-190 `marginal_quartic_running_reach_to_cell_scale`** and its 16 suite checks (the rate `c = 4γ₂/(16π²) = 1/(6π²)`; checks [6], [7], [10′], [10″], [12], [13]); **§C.5.2**'s one-loop-flow paragraph; **N71** (the whole failed-because, whose L-A reach and L-C domain ratio both carry `γ₂`); the worklist's closed ★★★ PROBE C row; companion Result Index row R-190 and dev log G.16. — **THAT IS THE COMPLETE BLAST RADIUS.** ★ **LOAD-BEARING, and how: the headline is INVERSELY PROPORTIONAL to `γ₂`** — `E = λ_S(cell)/c = 4π²/(e²γ₂) = 1.9937` e-folds — and the sign concordance with R-148 needs `γ₂ > 0`. **CONDITIONAL TIER ON EVERY DEPENDENT:** R-190's rate and e-fold count are `DERIVED-numeric` **given this READ `γ₂`**; the exactness theorem and the `λ_S = 4l₂` identification are `DERIVED-A` and do **not** ride this row | **INSIDE-FRAME / EFFECTIVE** — the chiral effective theory of the dressed coset Goldstones at the cell scale; the **same jurisdiction column as I-13**, and explicitly NOT a substrate-level statement (the grain-layer bond model is a different object — R-190's leg L-B refuses the identification) | Chiral `SU(2)×SU(2)/SU(2)` coset on the target `SU(2) ≅ S³`; 4D Minkowski; the `O(p²)` term present with coefficient `F²/4`; the GL renormalization scheme and Weinberg power counting; **and the domain hypothesis `μ ≲ Λ_χ ~ 4πF_π`**. **THE HYPOTHESES WERE CHECKED ONE BY ONE AGAINST THE BANKED ACTION AND FIVE OF THE SIX MATCH** (the coset, the dimension, the `O(p²)` term, the Minkowski sign anchored by R-148's `E₄ > 0`, the marginality) | **NAMED-CRACK — and the crack is NAMED BY THE IMPORTER, which is the point.** The **sixth hypothesis FAILS**: the requested run is ~37–42 e-folds and this import is valid for at most `ln 4π = 2.531`. That failure is not a hidden exposure — it is **banked as R-190's own leg L-D** (ranked below L-A/L-B as a validity fence on THIS import, not on the mechanism) and asserted in suite check [13]. Consequently the import **cannot be extrapolated** and no dependent claims it is. The `γᵢ` themselves are standard, uncontested, textbook ChPT; the crack is one of RANGE, not of correctness | The **kernel's own UV spectral computation — the same object that owes DGLAP** (I-13's identical handle, and R-148's Class-2 residual). Computing the running in the correct UV theory above `Λ_cell` retires this row and is exactly N71's would-change-if (2). **REVERT CLAUSE:** refusing the GL running strikes R-190's **rate and e-fold count** (the `DERIVED-numeric` half) and with them N71's quantitative L-A and L-C; what SURVIVES untouched is the `DERIVED-A` half — the `λ_S = 4l₂` identification, the no-quadratic-part series result, the exactness theorem, and **leg L-B, the referent argument, which uses no `γ₂` at all**. So a revert leaves N71 standing on L-B and loses its numbers |
...
| I-19 | **Dimension-six Lorentz-violation EFT constraint machinery** (the `E² = p² + m² + η⁽⁴⁾ p⁴/M²_Pl` parametrization and the published n = 4 bounds derived in it: Liberati 2013 eqs. 77–78 — photon `−10⁻⁷ ≲ ξ⁽⁴⁾ ≲ 10⁻⁸`, electron `−10⁻⁷ ≲ η⁽⁴⁾ ≲ 10⁻⁶`, proton `−10⁻³ ≲ η⁽⁴⁾_p ≲ 10⁻⁶` at 99% CL for pure-proton composition; Stecker 2009 eq. 18, `δ^π_p < 4.5 × 10⁻²³` from the Auger spectrum above the GZK energy). **RE-TABLED 2026-08-26 (E21-VERDICT):** the strong corners are CONDITIONAL — pure-proton composition conditions the photon and electron rows too, so **premise (d)'s scope is WIDENED to all species rows**, and the photon `10⁻⁸` corner is a **PROJECTED** bound ("not real constraints" in its authors' words; triggering UHE-photon detection unmet as of 2026); the UNCONDITIONAL bound is Auger 2022's model-independent `η⁽⁴⁾_p < 0.149` (superluminal, rigidity-rescaled). Dossier: `BOUNDS_DOSSIER_V-C_2026-08-26.md`; engine INPUT block re-tabled in the same pass | R-165, §B.1.5, §B.6.3, §B.6.4, §E.3.3 VG-6, §E.3.5(4) — the entire statement of the framework's dim-6 LV exposure — **and, since 2026-07-27, `eom_constraint_class`'s `E1_dim6_isotropic_LV_ceiling`**, the #1-gap compatible-field boundary's first empirical entry (kept in its own bucket, never renumbered `H12`, precisely so this excision fires against E1 alone and leaves H1–H11 untouched) | **Inside-frame data bridge** (sibling of I-6): the bounds are inferences from observed spectra *within* an effective-field-theory parametrization, imported as data, not derived | (a) an EFT description of propagation at the relevant energies; (b) the CPT-even dim-6 operator basis is the right parametrization; (c) the coefficients are defined in the CMB frame; (d) the UHECR composition assumed in the fits — WIDENED 2026-08-26 (E21): pure-proton conditions ALL the strong corners, the photon and electron rows included (they ride GZK-secondary photons; Liberati §7.5 in his own voice), not the proton rows alone; the composition-free bound is Auger 2022's `η⁽⁴⁾_p < 0.149`, and composition is UNSETTLED (near-zero proton fraction reported above 10¹⁹ eV — not a species ID; the corners re-arm if it resolves adversely); **(e) FRAME JURISDICTION** — that the measured `η⁽⁴⁾` may be identified with the *substrate's own* quartic dispersion coefficient. The bounds are INSIDE-frame inferences about propagating particles; the object bounded is the OUTSIDE-frame strain-mode dispersion (`Cl41Wave().wave_speed_c()`, which raises). The transfer runs through the un-built outside↔inside projection; the SCALE half of the same crossing (`a ↔ ℓ_Pl`) is the renormalization dictionary's length/scale face (worklist dictionary item, sixth face, 2026-08-16) | **OPEN as derivation, SANCTIONED as data import** — the same status as I-6. Premise (c) closes the **inertial-frame** question only, and there it is a coherence *win* rather than a crack: TWT's τ₅-foliation IS the comoving/CMB frame (§B.4.5), so the frames agree. The **frame-jurisdiction** question is separate and is premise (e) — the same crack §E.3.1 rows 7–8 and §E.5 already carry for inside-frame laboratory limits binding outside-frame floors; it does not weaken the case for NAMING the exposure (canon §0a), but the exposure's BINDINGNESS is itself conditional and saying so costs nothing. Premise (d) is the DOMINANT conditioning (E21): it is the entire difference between the ~1-order unconditional exclusion and the 6–7-order conditional corners — not a small loosening of a large gap (the gap size is `Λ`-dependent: the 2026-07-30 which-Λ ruling assigns dispersion consumers `Λ_L = 1/a ∈ [0.39, 0.73] M_Pl`, naive `η⁽⁴⁾ ∈ [1.9, 6.7]` — R-037) | A TWT-native computation of the substrate strain-mode dispersion (the #1 gap, §D.5) — which would let the framework state its own `η⁽⁴⁾` and read the bounds directly rather than through the EFT parametrization. **REVERT CLAUSE:** strike this row and the *positive* half of R-165 (D4 fourth-moment isotropy ⇒ dim-8 anisotropy) is UNAFFECTED — it is a pure lattice identity with no import. What falls is the *exposure* statement: VG-6, §E.3.5(4) and N52 revert to "the dim-6 isotropic coefficient is uncomputed and untested", and §E.3.1 does not regain its deleted rows 1–2 (those were deleted for reading a dim-6 object against a dim-4 bound — an error independent of this import) |
...
| I-22 | **Universality-plus-rescaling removal of a species-universal dimension-four Lorentz-violation coefficient** (a maximum speed common to all species is absorbed by a coordinate/units rescaling and carries no relative observable — standard LV-EFT lore, sibling of I-19; the *conclusion* is the import, the algebra of the rescaling is not) | §B.1.5 — the loop-order-independent half of the R-016 defusal (restored from V1 §12.6, 2026-07-30); the dimension-four scope of R-016/R-039; §B.6.3's dimension-four protection statement ("The dimension-four boost bound is closed by matter-as-defect") | **Substrate-level** — the loops run over substrate modes and the rescaling is applied to the substrate's own light-cone, i.e. it sits on the borrowed-mechanism side of the 13.4 discriminator, not the data-like side | (a) the ONE substrate field generates all the radiative corrections, so the induced dim-4 coefficient is genuinely common to every defect species — the premise the whole step rides, and the one that is not engine-checkable; (b) a loop/derivative expansion over substrate modes exists at all (a non-analytic #1-gap memory kernel leaves 'loop order' undefined and the argument's framing lapses); (c) the removed coefficient is a *constant*, not momentum-dependent — which is exactly why the step does NOT extend to dimension six | **OPEN** — premise (a) is a substrate claim only the #1-gap kernel can confirm; premise (b) is the #1 gap itself. Recorded at DEFUSAL tier: the step closes the dim-4 exposure *conditionally* and never claimed the substrate's interacting Lorentz invariance, which V1 §12.6 conceded open in the same paragraph. V1 carried this reasoning at a flat DERIVED tag; that tag is deliberately not restored with it. **AMENDED 2026-08-24 (round-4, O3): premise (a) is not neutral-pending; the best available external evidence runs against it.** The one worked substrate programme in the literature obtains **species-dependent effective metrics generically** for its own medium and recovers a single metric only where a **vacuum symmetry** connects the species; its author's own infrared-attractor fallback he judges quantitatively insufficient *(our own primary read, `PROBE_SPECTRAL_NODE_2026-08-23.md` §2.2 — recorded there as a cost of the spectral branch and equally an argument against this premise)*. The family's own multiple-defect-axes statement points the same way. **Status of premise (a) is therefore UNSUPPORTED, not merely unchecked** — and *unsupported* is not *refuted*: our defects do not sit at distinct nodes, so whether the analogue's mechanism transfers is itself open. Paper-side consequence, applied in the same pass: Core §2.5 states the premise as **load-bearing** rather than as a rider, and Core §3.3 counts the one-medium light cone as a **conditional** counterweight against the point-group **theorem** | **Retirement handle — amended 2026-08-24 (round-4, F4(b)); the row previously named only (i).** *(i)* The #1-gap kernel's own dispersion computation (§D.5) — the same object I-19 waits on. *(ii)* **NEW, and the cheaper of the two: the field-reclamation debt (Core §4.4).** Premise (a) is the proposition that every field is a mode of the one medium, and **gauge structure is on that debt's un-reclaimed list** — so premise (a) is not payable while the gauge sector is un-reclaimed, and it becomes payable *without* the kernel the moment the photon is exhibited as a collective mode of the substrate with universal coupling. These are the same premise seen from two debts. *(iii)* **Possible identity with the orbit-constancy discharge object (CANDIDATE, unverified — not a third retirement route until it is verified):** the Ward-type identity that would force the dressed bond weights orbit-constant despite the drive may be this same premise. If it is, the two debts are one and both discharge together. Check before the two are worked as separate arcs (`TWT_worklist.md`, the ★★ orbit-constancy row, which carries the forward half of this pointer). **REVERT CLAUSE:** strike this row and §B.1.5's *relative-boost* half is UNAFFECTED (R-016's one-field/one-light-cone statement is import-free — it is a claim about which coefficient *exists*, not about what cancels). What falls is the *radiative* half: the Collins-class obstacle reverts from 'defused at its precondition, independently of loop order' to 'defused at the classical/tree level, with the radiative case named open'. §E.3.5(4), VG-6, N52 and I-19 do not move |
...
| I-30 | **Pati & Salam, "Lepton number as the fourth 'color'"**, *Phys. Rev. D* **10**, 275 (1974) [erratum *Phys. Rev. D* **11**, 703 (1975)]. **READ AT PRIMARY BY OUR OWN TOOLS 2026-08-24** — the ICTP preprint IC/74/7 (Jan 1974, full 38-page scan, six pages re-verified as page images); the published PRD body was **unreachable** (APS 403), though the published abstract was read verbatim and matches; **the 1975 erratum was NOT read.** Record: `knowledge/audit/external_review_r4_2026-08-24/primary_reads/READ_PATI_SALAM_1974.md`. **The `[PREPRINT-READ]` qualifier travels with every quotation of this row.** What the primary supports — and only this: **(1) the fourth-colour thesis**, quarks carrying four colours, three baryonic and a fourth that "represents lepton number L", the colour gauge symmetry `SU(3')` extended to `SU(4')`, the fermion array's fourth-colour column being the lepton quartet. **(2) `SU(4) ⊃ SU(3) × U(1)`**, explicit — but **the label `U(1)_{B−L}` is not the paper's own**: it names the `SU(4')` generator `F'₁₅` (calling it, by flavour analogy, the "charm-generator"), and **the paper normalizes `B = 1` per quark** (proton `B = 3`), not the modern `1/3`; quoting a "B−L" statement from it without that normalization is a live referent-drift hazard. **(3) The paper PREDICTS proton decay** — consequence (3) of its own abstract is "baryon-lepton number violation in quark and proton decays", and §6.7 gives `p → 3ν + π⁺`, `4ν + e⁺` and similar. **(4) What is exactly conserved throughout is `F = B + L`, not `B − L`** — in the integer-charge branch the paper mainly works, spontaneous W–X mixing violates B and L; `B − L` conservation holds only in the fractional-charge branch, where it is *derived* from fermion-number plus electric-charge conservation, so it is **branch-dependent in 1974**, not a property of the scheme. **(5) `p → e⁺π⁰` is excluded by selection rule** — "no two- or three-body decays are allowed", on fermion-number grounds. **(6) `K → μe`** is listed as a "muon-electron number-violating transition", explicitly "relevant for the 'basic' model only" — do **not** quote it as `K_L → μ±e∓`, and do not call the *process* `ΔB = ΔL` (the vertex is; the process is not). **(7) `ν_R` exists** as the fourth-colour partner, both chiralities being 16-folds, the neutrinos initially "4-component objects". **(8) Dirac neutrinos may NOT be attributed to this paper** — §5.2 calls the situation "a dilemma of massive neutrinos" and adds two gauge-singlet left-handed fields to force massless 2-component physical neutrinos; the final model carries "16 + 16 + 2 = 34 2-component fields". **(9) The "16th Weyl seat" framing is NOT available from this paper** — its 16-fold is 4 valency × 4 colour and its lepton quartet spans two lepton generations; the `SO(10)`-style one-generation 16 is a later structure and must be attributed elsewhere | **NO load-bearing use — prior-art registration.** The kinship bears at: Core §2.1's `{3, +} / {1, −}` trivector pattern and its `3 × 1/3 = 1` arithmetic, where the row is cited in the paper; Core §2.6's `B − L` triad; §3.1's separator grading of both | **N/A — no conclusion is used** | **None consumed** | **PRIOR ART — SAME ARITHMETIC, different ontology.** What is *not* ours: the 3+1 split, and lepton number read as a fourth colour. What *is* ours: obtaining that split from a trivector orbit structure with **no gauge group, no breaking sector and no leptoquark**. Describe it as a joint, never as an unprecedented arithmetic. **STILL QUARANTINED, and this fence is load-bearing:** the modern "minimal Pati–Salam" statement — gauged `SU(4)_c × SU(2)_L × SU(2)_R`, `B − L` conserved, no proton decay — **is not sourced to this paper, and the read establishes that it cannot be.** It may well be true of the modern model; it **needs its own primary before any sentence in the corpus leans on it**, and the round-4 merge's sharpening ("minimal P–S conserves B−L exactly and does not predict proton decay") is therefore **NOT applied to Core §2.6**, whose "unification expects decay at *some* level" survives the 1974 primary intact. Also still unread: the published PRD body at page level, and the 1975 erratum, with residual unquantified risk that the erratum touches any of the above | None needed while nothing rides it. **Promotion trigger:** any TWT use of a P–S *result*, or any quoted flavour bound from that literature — either enters 13.1 with its own premises and requires the published body first, and a modern-minimal claim requires a modern primary of its own |
| I-31 | **The lattice chiral-fermion no-go and the symmetric-mass-generation line** *(carried under the alias `I-NN2` in the negatives ledger N67/N68 and in the round-4 records; one row, two names, no second registration)*. **(a) Nielsen–Ninomiya** — a local, translation-invariant lattice theory whose Hamiltonian is **free / quadratic in the fields** and invariant under a phase change of the fermion field admits no odd number of Weyl species; the doublers come in pairs (Nielsen & Ninomiya, *Phys. Lett. B* **105**, 219 (1981); *Nucl. Phys. B* **185**, 20 and **193**, 173 (1981)). **Hypotheses verified at primary via Friedan, *Commun. Math. Phys.* **85**, 481 (1982)** — condition (1) reads "quadratic in the fields"; the 1981 originals were genuinely unreachable and are tagged as such. **(b) Symmetric mass generation** — the conjecture that a lattice/bosonic system can gap a mirror sector without breaking the symmetry, and so non-perturbatively define a chiral gauge theory, **iff** the chiral content is free of all 't Hooft anomalies including the discrete and global ones (Wen, *Chin. Phys. Lett.* **30**, 111101 (2013), arXiv:1305.1045; Wang & Wen, *Phys. Rev. Research* **2**, 023356 (2020), arXiv:1809.11171; You, BenTov & Xu, arXiv:1402.4151; García-Etxebarria & Montero, *JHEP* **08** (2019) 003, arXiv:1808.00009). **Primary-read status, 2026-08-24** (`knowledge/audit/external_review_r4_2026-08-24/primary_reads/READ_NN_SMG_2026-08-24.md`): (a) and the Wen / Wang–Wen / YBX / GEM statements are **VERIFIED AT PRIMARY**; **Eichten–Preskill (1986) and the constructive-evidence line were NOT read this pass and stay quarantined at `[SNIP]`**, so nothing from them may be quoted | **NO load-bearing use — obstruction-shape and prior-art registration — EXCEPT: R-183's leg 1 (the sterile cross-brace, BANKED-PENDING-§8a) reads this row's `I-SMG-4`/`I-SMG-5` sub-rows as its bosonic-lattice-license leg at conjecture strength; striking or re-siding those sub-rows fires R-183's revert clause (keeper closing round 2026-08-24 — the excisability edge restored from this side).** Bears at: Core §2.4's fourth fence, where the theorem is named and its hypotheses stated as having no referent here; the emergent-fermion sector (§C.1, FR quantization, V3 pick 11); the D4 grain's dispersion (§B.1.5 and the spectral probe's real-class measurement); Core §4.4's field-reclamation debt; and any future statement that the substrate hosts chiral matter | **Substrate-level** — a lattice/regularization statement about the grain layer, not an inside-frame effective one | **(a)** locality, translation invariance, a conserved fermion/chiral phase, and — the hypothesis the whole question turns on — a **free, quadratic lattice fermion action**. *(Hermiticity is not carried as an independent premise: it is implicit in the self-adjointness of `H`, and the commission draft's premise cell over-stated it. Repair applied at registration, per the primary read §1b.)* **(b)** the conjecture's own status **as a conjecture**, plus the anomaly conditions in their strong, discrete-and-global form | **OPEN — and registered *against* us as much as for us.** What the record supports today: TWT has **no lattice fermion field**, so NN's hypotheses have no referent in the banked substrate — **which is not an evasion**, because the corpus has not exhibited a chiral spectrum from the substrate at all (RUL-049 conditioning travels with every quotation). The **Fermi-point evasion is measured unavailable on the banked substrate**: `PROBE_SPECTRAL_NODE_2026-08-23.md` places the banked D4 Bloch dynamics in the **real (orthogonal) symmetry class**, with the gapless set exactly the Goldstone triplet, no node at finite `k`, Chern number identically zero on a calibrated instrument, and the only available momentum-space invariant valued in **ℤ₂** — the wrong coefficient group for `ℤ⊕ℤ`; and Volovik's own Standard-Model node is marginal, `N₃ = 8 − 8 = 0`, so the route supplies no protection at the target even where it is formulable. TWT's per-generation content is the anomaly-free 15 gauged Weyl + 1 sterile = **16** (the `SO(10)` spinor), satisfying the conjecture's antecedent — but anomaly freedom is **partly imported** (I-17/I-18's hypercharge conditions) and only the `B − L` condition is native (§C.5.4). **A favourable coincidence of content is not an argument and is never to be quoted as support.** **Scope correction taken from the primary (GEM):** the ℤ₁₆ constraint binds the **total** Weyl count, not each generation separately — "generation by generation" must not be written. **Strengthening from the same primary:** GEM's own stated escape hatch is a Majorana mass for `ν_R`, which this family's exact `B − L` structurally forbids, so the consonance is **structural rather than a numerical coincidence**. **The generalized no-go is annotated at N67** (Golterman–Shamir, PRD 113 (2026) 014503), adjudicated **DOES-NOT-APPLY-AS-PROVED** — its assumption (1) quantifies over fermion-field variables with no referent here — **with the mechanism surviving as a design constraint** the solitonic route must positively evade; use that adjudication's wording, not the read dossier's | The substrate's own derivation of its chiral spectrum — the same unbuilt object Core §4.4's field-reclamation debt names. **Strike the row and nothing falls**, since nothing rides it; what is lost is the record that the sharpest known obstruction-shape for this family's matter sector, and the strongest external precedent for it, are the same literature. **Promotion trigger:** any use of an SMG *result* as support, which requires the unread primaries (Eichten–Preskill; the 3+1D constructive-evidence line) first |

**The `I-SMG` sub-rows of I-31, registered as amended (2026-08-24).** The round-4 commission
submitted a six-row breakdown of the SMG line (`I-SMG-1 … I-SMG-6`). It is carried here **as
sub-rows of I-31 rather than as six registry IDs**, because nothing rides any of them and the
package is quarantined as a whole. **Three repairs from the primary read are applied at
registration and must travel with any restatement** — each runs in a different direction and two of
the three correct the submission in this programme's own favour, which is why they are stated rather
than absorbed.

| sub-row | as submitted | as registered, after the primary read |
|---|---|---|
| `I-SMG-1` | premises "locality, translation invariance, Hermiticity" | **REPAIRED:** the premise the whole question turns on is the **free / quadratic** fermion action (Friedan's condition (1)); **Hermiticity is dropped as an independent premise**, being implicit in the self-adjointness of `H`. Statement and conclusion verified at primary |
| `I-SMG-2` | Eichten–Preskill 1986 and its failure (Golterman–Petcher–Rivas 1993) | **UNCHANGED and STILL QUARANTINED at `[SNIP]`** — not read this pass; no statement from it may be quoted |
| `I-SMG-3` | mirror-decoupling "contested", with **Kikukawa ×2 listed among the opposition** | **REPAIRED — and the repair is a SPLIT, not a deletion (the reviewer's own wording offer, adopted).** **(i) STANCE:** both Kikukawa papers are **constructive, on the PROPONENT side** — two of the four cited opponents were mis-sided, and the author must be attributed proponent-side wherever he is cited. **(ii) RESULT-CONTENT, separately and still citable in the OBSTRUCTION half:** PTEP **073B02**'s *failure diagnosis* — its own account of where the construction does not close — remains quotable as obstruction-shape evidence, because a proponent's diagnosis of his programme's failure mode is evidence about the obstruction regardless of the stance it is offered in. **These two are never to be collapsed:** citing 073B02's diagnosis does NOT convert Kikukawa into an opponent, and attributing him proponent-side does NOT retire the diagnosis. **(iii) THE GENUINE NEGATIVES**, unchanged: **Chen–Giedt–Poppitz**; **Golterman–Shamir** remain the live opposition (verified at primary, both papers). The "2026 PRD" journal reference attached to arXiv:2505.20436 is **unverified** and is not to be quoted as a journal reference |
| `I-SMG-4` | "the 15n (no-`ν_R`) Standard Models **require extra structure**" | **REPAIRED, and the true statement is more favourable to a bosonic substrate:** the primary's distinction is **fermionic-lattice versus bosonic/qubit-lattice**, not "extra structure". The conjecture-label referent is corrected with it: Wen's statement is a construction claim, not a demonstration, and must be cited as a conjecture with named opposition |
| `I-SMG-5` | `ν = 0 mod 16` per generation | **AMENDED:** fully primary-verified, and the **strongest** row in the read — with two scope fixes carried into I-31's ontology cell: the constraint binds the **total** count (so "generation by generation" is struck), and **GEM's own `ν_R`-Majorana escape hatch** is what makes this family's exact `B − L` load-bearing rather than coincidental |
| `I-SMG-6` | constructive SMG evidence, 1+1D and 3+1D | **UNCHANGED and STILL QUARANTINED at `[SNIP]`** — not read this pass. Recorded against it: the opposition argues at length that the lessons drawn from two-dimensional models are limited, which bears directly on what the 1+1D existence proof is worth for 3+1D |

*Two further open items ride this block and are recorded, not resolved:* **OI-2**, the discrete-`ℤ₃`
colour cobordism computation, is filed as **N68**, with the read's cheap first move noted there (if
the `ℤ₃` embeds in an anomaly-free `U(1)`, the item discharges free); and **OI-3**, Wang–Wen's
gauge-charge condition on fermions emerging from a bosonic substrate, is a **new falsifiable
condition not present in the submitted dossier** and is recorded here as owed.
...
- **PINNED** — an instance parameter of V3, no endorsement implied. Ten at the stamping pass;
  **eleven nodes in the tree today** (`V3-1 … V3-11`, plus the sub-nodes `V3-1a`, `V3-1b`,
  `V3-2a` and `V3-10a`). Both numbers are
...
The same sweep classified §E.3.1's sixteen rows. **Fourteen are family-level; two are
instance-level** (RUL-081's enacted fourteen/two split — this sentence corrected 2026-08-24, §8a
batch keeper C4; it previously read fifteen/one, a site the ruling's six-site sweep missed) —
row 6 and row 12 (no baryon containing a top quark), the latter's kill number `Γ_t · Θ_0 ≈ 7.2`
riding the ANW inertia and hence the pinned hadron toolbox and calibrations. Rows 4 and 5 are
