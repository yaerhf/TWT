...
| **The renormalization dictionary** (missing object; named 2026-07-31) | which inside-frame scheme/scale a substrate-level quantity lands on | The outside↔inside projection OWES this entry. Two known faces: **N57** (`ω` ↔ which mass definition — Koide exact at pole, degraded ~2.5 orders at MS-bar) and **C-7** (`√18/(D/J)` ↔ which scheme's `e`). Until the entry exists, cross-frame numeric agreements cannot be fully credited nor discrepancies interpreted. Worklist item (ledger). |
