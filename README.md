...
Expected output: `ALL 507 CHECKS PASSED across 10 modules.` (On Windows, set `PYTHONUTF8=1` first.)
...
| `twt.py` | The substrate oracle, main engine — the dossier's spine. Clifford `Cl(4,0)` / `Cl(4,1)` algebra, the D4 lattice layer, spinor and grade machinery, and one function per banked result. Each carries a docstring stating its **status tier** and the premises it consumes. Since the family split this file is a thin **facade**: it defines nothing and re-exports the two halves below, so `import twt` gives the whole surface. |
